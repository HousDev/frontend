...
const DashboardLayout = () => {
  ...
  // Auto-expand menu if current route matches any child
  useEffect(() => {
    const currentPath = location.pathname;

    // Check which parent menu should be expanded based on current route
    const menuMappings = {
      'cms': ['/dashboard/blog-manager', '/dashboard/document-center', '/dashboard/template-center'],
      'crm': ['/dashboard/leads', '/dashboard/buyers', '/dashboard/sellers', '/dashboard/communication', '/dashboard/activities'],
      'tools': ['/dashboard/analytics', '/dashboard/accounts', '/dashboard/vendors'],
      'others': ['/dashboard/contact-messages', '/dashboard/ai-training']
    };

    Object.entries(menuMappings).forEach(([menuKey, paths]) => {
      if (paths.some(path => currentPath.startsWith(path))) {
        setExpandedMenus(prev => new Set([...prev, menuKey]));
      }
    });
  }, [location.pathname]);
    ...
    structure.push(
      {
        ...
        key: 'cms',
        children: [
          { name: 'Blog Manager', href: '/dashboard/blog-manager', icon: Edit3, color: 'text-rose-600' },
          { name: 'Document Center', href: '/dashboard/document-center', icon: FileText, color: 'text-amber-600' },
          { name: 'Template Center', href: '/dashboard/template-center', icon: LayoutTemplate, color: 'text-lime-600' },
        ]
      },
      {
        ...
        key: 'crm',
        children: [
          { name: 'Leads', href: '/dashboard/leads', icon: Target, color: 'text-emerald-600' },
          { name: 'Buyers', href: '/dashboard/buyers', icon: UserCheck, color: 'text-teal-600' },
          { name: 'Sellers', href: '/dashboard/sellers', icon: Users, color: 'text-cyan-600' },
          { name: 'Communication', href: '/dashboard/communication', icon: MessageSquare, color: 'text-sky-600' },
          { name: 'Activities', href: '/dashboard/activities', icon: Activity, color: 'text-red-600' },
  ...
  const filteredNavigation = useMemo(() => {
    ...
    navigationStructure.forEach(item => {
      if (item.type === 'single') {
        ...
        if (item.name.toLowerCase().includes(searchQuery.toLowerCase()) || matchingChildren.length > 0) {
          filtered.push({
            ...item,
  ...
  const isParentActive = useCallback((children) => {
  ...
  // Enhanced Sidebar component with dropdown functionality
  const SidebarComponent = useMemo(() => {
    return (
      <div className="flex flex-col h-full" ref={sidebarRef}>
        {/* Company Header */}
        <div className="flex items-center h-16 px-6 border-b bg-gradient-to-r from-blue-50 to-indigo-50">
          {companyLogo ? (
            <img
              src={companyLogo}
              alt="Company Logo"
              ...
              {item.type === 'single' ? (
                ...
                <div>
                  ...
                  <button
                    ...
                    className={cn(
                      'group flex items-center w-full px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 hover:translate-x-1',
                  ...
                  >
                    <item.icon
                      className={cn(
                        ...
                        isParentActive(item.children)
                          ? 'text-white'
                          : `${item.color} group-hover:${item.color.replace('text-', 'text-')}`
                      )}
                    />
                    ...
                    {expandedMenus.has(item.key) ? (
                      <ChevronDown className={cn(
                        ...
                        isParentActive(item.children) ? 'text-white' : 'text-gray-400'
                      )} />
                    ) : (
                    ...
                    <div className="ml-6 mt-1 space-y-1 border-l-2 border-gray-200 pl-4">
                      {item.children.map((child) => (
                        <Link
                          ...
                          onClick={handleSidebarLinkClick}
                          className={cn(
                            'group flex items-center px-3 py-2 text-sm font-medium rounded-lg transition-all duration-200 hover:translate-x-1',
                            isActive(child.href)
                        ...
                        >
                          <child.icon
                            className={cn(
                              ...
                              isActive(child.href)
                                ? 'text-white'
                                : `${child.color} group-hover:${child.color.replace('text-', 'text-')}`
                            )}
                          />
