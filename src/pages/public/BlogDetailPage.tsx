...
const isPublicFromRaw = (p: any) => {
  ...
  if (has("draft")) return !p.draft; // draft=true => not public
    ...
    const loadRelatedAndMeta = async () => {
      try {
        ...
        const normalized: BlogPost[] = (list || []).map((p: any, idx: number) => {
          ...
          // ADDED: carry a non-typed flag for visibility
          (obj as any).__isPublic = isPublicFromRaw(p);
          return obj;
        });

        if (!cancelled) {
          // ADDED: only public posts
          ...
          if (post) {
            // ADDED: candidates only from public posts
