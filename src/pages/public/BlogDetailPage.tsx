...
const BlogDetailPage: React.FC<BlogDetailPageProps> = ({
  ...
  const viewRecordedRef = React.useRef<string | number | null>(null);

useEffect(() => {
  if (!post?.id) return;
  if (viewRecordedRef.current === post.id) return; // already recorded this post in this mount
  viewRecordedRef.current = post.id;
  blogsAPI.recordView(post.id).catch(() => {});
}, [post?.id]);
 ...
 const handleLike = async () => {
    ...
    try {
      if (blogsAPI.likePost) {
        const res: any = await blogsAPI.likePost(post.id);
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-3 sm:gap-4">
                        ...
                        >
                          <img
                            src={r.image || getDefaultBlogImageByCategory(r.category)}
                            alt={r.title}
                            className="w-full h-full object-cover group-hover/image:opacity-90 transition"
                            onError={(e) => {
                              // Fallback if image fails
                              const target = e.currentTarget as HTMLImageElement;
                              target.src = getDefaultBlogImageByCategory(r.category);
                            }}
                          />
                        </Link>
              ...
              {recentPosts.map((r) => {
                ...
                return (
                  <div key={r.id} className="flex items-start gap-2 sm:gap-3 group">
                    {/* ✅ Recent post image with fallback */}
                    <Link
                      to={to}
