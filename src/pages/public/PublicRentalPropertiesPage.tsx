...
import {
  toggleTenantShortlist,
  getTenantShortlist,
} from "@/lib/tenantShortlist";
            ...
            ? () => (
                <span className="text-xs mr-1">
                  {style.emoji ? (
                    typeof style.emoji === "string" ? (
                      <span
                        className="text-xs mr-1 uppercase"
                        aria-hidden="true"
                      >
                        {style.emoji}
                      </span>
...
/* ==============================
   Component
============================== */
const PublicRentalPropertiesPage: React.FC<{
  onPropertyView?: (p: any) => void;
}> = ({ onPropertyView }) => {
  ...
  const [modalDefaultAction, setModalDefaultAction] = useState<
    "contact" | "schedule" | "shortlist"
  >("shortlist");
  ...
  const handleToggleShortlist = (e: React.MouseEvent, prop: any) => {
    ...
    if (!isLoggedIn) {
      ...
      toast.info(
        "Please verify your email via OTP to save this rental property to your Shortlist.",
      );
      return;
    }
    ...
    toast.success(
      nowShortlisted
        ? "Property shortlisted & saved to your Tenant Account!"
  ...
  useEffect(() => {
    ...
    const fetchCounts = async () => {
      try {
        ...
        console.warn(
          "Failed to load buy/rent counts in PublicRentalPropertiesPage:",
          e,
        );
      }
    };
  ...
  const cityOptions = masterCity.map((o) => ({
    value: o.value,
    label: o.label,
  }));

  const budgetMaster: any[] = findMasterOptions([
    "price range",
    "price_range",
    "budget",
    "budget range",
  ...
  const budgetOptions = budgetMaster.map((o) => ({
    value: o.value,
    label: o.label,
  }));

  const propertyTypesMaster: any[] = findMasterOptions([
    "property type",
    "property_type",
    "type",
    "place type",
  ...
  const propertyTypeOptions = propertyTypesMaster.map((o) => ({
    value: o.value || o.label,
    label: o.label || o.value,
  }));

  // ✅ Compute dynamic property types based on actual loaded rental properties
  ...
  const propertySubtypesMaster: any[] = findMasterOptions([
    "property subtype",
    "property_subtype",
    "subtype",
    "unit subtype",
  ]);
  const propertySubtypeOptions = propertySubtypesMaster.map((o) => ({
    value: o.value || o.label,
    label: o.label || o.value,
  }));

  const unitTypesMaster: any[] = findMasterOptions([
    "unit type",
    "unit_type",
    "unit",
    "bhk",
  ...
  const unitTypeOptions = unitTypesMaster
    .map((o) => ({ value: o.value || o.label, label: o.label || o.value }))
    .filter(Boolean);

  const bedroomsMaster: any[] = findMasterOptions([
    "bedrooms",
    "bhk",
    "beds",
    "unit type",
  ...
  const masterLocation: MasterOption[] = findMasterOptions([
    "location",
    "locality",
    "localities",
    "area",
  ...
  const loadPropertiesFromSearch = useCallback(async () => {
    ...
    try {
      ...
      const buildAdvancedParams = () => {
        ...
        if (qp.get("transaction"))
          params.transaction_type = qp.get("transaction");
      ...
      const buildSimpleParams = () => {
        ...
        if (qp.get("transaction"))
          simpleParams.transaction_type = qp.get("transaction");
        return simpleParams;
      };
      ...
      let list: any[] = [];
      const activeAPI =
        transactionType === "rent" ? rentalPropertiesAPI : propertiesAPI;
  ...
  const handleNavigateToProperty = async (property: Property) => {
    ...
    if (isLocked) {
      const redirectUrl = property.slug
        ? `/rentals/${encodeURIComponent(String(property.slug))}`
        : "/rentals";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-lg text-blue-100 mb-2 max-w-2xl mx-auto">
              Discover verified rental properties from trusted landlords across
              top locations
            </p>
                  ...
                  >
                    <div className="flex gap-2 py-1 snap-x snap-mandatory">
                      <button
                        ...
                        className={`shrink-0 snap-start whitespace-nowrap px-4 py-1.5 rounded-full text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-white/70 transition cursor-pointer inline-flex items-center gap-1.5
                      ...
                      <div className="relative">
                        ...
                        <div className="absolute top-3 right-3 flex items-center gap-2 z-20">
                          <button
                            ...
                            title={
                              likedProperties.includes(property.id.toString())
                      ...
                      <div className="p-6 flex-1 flex flex-col">
                        ...
                        <div className="flex flex-wrap gap-1 mb-3">
                          {shownAmenities.map((amenity, i) => (
                            <div
                              key={i}
                              className="flex items-center space-x-1 px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full text-xs"
                            >
                          ...
                          <div className="flex items-center justify-between pt-1.5 sm:pt-2 border-t border-gray-100">
                            ...
                            <div className="flex items-center gap-1 sm:gap-1.5 flex-shrink-0 ml-auto sm:ml-0">
                              ...
                              <button
                                onClick={(e) => {
                                  ...
                                  const priceText = !isNaN(
                                    ...
                                    : "Price on request";
                                  const link = property?.slug
                                    ? `${window.location.origin}/rentals/${encodeURIComponent(String(property.slug))}`
                                    : `${window.location.origin}/rentals`;
                                  const message = `Hi, I'm interested in ${title} at ${loc}. Price: ${priceText}. Can you share more details?\n${link}`;
                                  window.open(
                              ...
                              <button
                                type="button"
                                onClick={(e) =>
                                  handleToggleShortlist(e, property)
                                }
