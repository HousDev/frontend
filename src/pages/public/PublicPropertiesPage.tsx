...
const isNewlyAddedProp = (p: any, daysThreshold = 3): boolean => {
  const dateVal =
    p?.created_at || p?.createdAt || p?.publication_date || p?.date_added;
...
const extractUnitType = (p: Property) => {
  ...
  for (const c of candidates) {
    const m = c.match(
      /(\d+(?:\.\d+)?\s*BHK|\d+(?:\.\d+)?\s*RK|studio|penthouse|duplex|bungalow|villa)/i,
    );
    if (m) {
  ...
  if (p.bedrooms && Number.isFinite(p.bedrooms) && p.bedrooms > 0)
...
const composeHeaderTitle = (p: Property) => {
  const parts: string[] = [];
  const type = (
    (p as any)._raw?.property_type_name ||
    p.property_type ||
    (p as any)._raw?.property_type ||
    ""
  )
    .toString()
    .trim();
  ...
  if (
    subtype &&
    !parts.some((pt) => pt.toLowerCase() === subtype.toLowerCase()) &&
    subtype.toLowerCase() !== unit.toLowerCase()
  ) {
    parts.push(subtype);
  }
...
const formatUnitAreaLine = (p: Property) => {
  ...
  const area =
    p.area ||
    p.square_feet ||
    (p as any)._raw?.carpet_area ||
    (p as any)._raw?.builtup_area;
  const areaText = area
    ? `${Number(area)
        .toFixed(area % 1 === 0 ? 0 : 2)
        .replace(/\.0+$/, "")} sq ft`
    : "";
...
const splitLocationCity = (p: Property) => {
  ...
  const parts = loc
    ...
    .filter(Boolean);
  if (parts.length === 1)
    return { locationPart: parts[0], cityPart: p.city || "" };
...
const parseBudgetRange = (value: string): [number, number] => {
  ...
  if (v.includes("-")) {
    ...
    return [
      isNaN(min) ? Number.NEGATIVE_INFINITY : min,
      isNaN(max) ? Number.POSITIVE_INFINITY : max,
    ];
  }
  if (v.endsWith("+")) {
    const tok = v.replace(/\+$/, "");
...
const matchesBudget = (property: Property, budget: string) => {
  ...
  return (
    price >= (isFinite(min) ? min : Number.NEGATIVE_INFINITY) &&
...
const extractParkingTypes = (p: any) => {
  ...
  if (raw.parking_types) {
    if (Array.isArray(raw.parking_types))
      parkingTypes = raw.parking_types.map((s: string) =>
        s.toString().toLowerCase(),
      );
    else if (typeof raw.parking_types === "string")
      parkingTypes = raw.parking_types
        .split(",")
        .map((s: string) => s.trim().toLowerCase());
  }
  if (raw.parking_details) {
    if (typeof raw.parking_details === "string")
      parkingTypes = parkingTypes.concat(
        raw.parking_details
          .split(",")
          .map((s: string) => s.trim().toLowerCase()),
      );
    else if (Array.isArray(raw.parking_details))
      parkingTypes = parkingTypes.concat(
        raw.parking_details.map((s: string) => s.toString().toLowerCase()),
      );
  }
  ...
  if (Array.isArray(p.amenities)) {
    p.amenities.forEach((a: string) => {
      ...
      if (
        low.includes("2 wheeler") ||
        low.includes("2-wheeler") ||
        low.includes("two wheeler")
      )
        parkingTypes.push("2w");
      if (
        low.includes("4 wheeler") ||
        low.includes("4-wheeler") ||
        low.includes("four wheeler") ||
        low.includes("car parking")
      )
            ...
            ? () => (
                <span className="text-xs mr-1">
                  {style.emoji ? (
                    typeof style.emoji === "string" ? (
                      <span
                        className="text-[10px] mr-1 uppercase"
                        aria-hidden="true"
                      >
                        {style.emoji}
                      </span>
...
/* ==============================
   Component
============================== */
const PublicPropertiesPage: React.FC<{ onPropertyView?: (p: any) => void }> = ({
  onPropertyView,
}) => {
  ...
  const [viewedProperties, setViewedProperties] = useState<Set<number>>(
    new Set(),
  );

  const [minRating, setMinRating] = useState<number | null>(null);
  const [possessionFilter, setPossessionFilter] = useState("");
  const [parkingFilter, setParkingFilter] = useState<"any" | "2w" | "4w">(
    "any",
  );
    ...
    const fetchCounts = async () => {
      try {
        const [buyRes, rentRes] = await Promise.allSettled([
          propertiesAPI.PublicgetProperties({
            status: "Available",
            isPublic: true,
            is_public: 1,
            visibility: "public",
          ...
          rentalPropertiesAPI.PublicgetProperties({
            status: "Available",
            isPublic: true,
            is_public: 1,
            visibility: "public",
        ...
        if (buyRes.status === "fulfilled") {
          const raw = Array.isArray(buyRes.value?.data)
            ? buyRes.value.data
            : Array.isArray(buyRes.value)
              ? buyRes.value
              : [];
        ...
        if (rentRes.status === "fulfilled") {
          const raw = Array.isArray(rentRes.value?.data)
            ? rentRes.value.data
            : Array.isArray(rentRes.value)
              ? rentRes.value
              : [];
          const publicProps = raw.filter(isPublicProp);
        ...
        console.warn(
          "Failed to load buy/rent counts in PublicPropertiesPage:",
          e,
        );
      }
    };
  ...
  const filterParamKey = queryParams.has("filterToken")
    ? "filterToken"
    : queryParams.has("fltcnt")
      ? "fltcnt"
      : undefined;
  const filterTokenFromUrl = filterParamKey
    ? ((queryParams.get(filterParamKey) as string | null) ?? undefined)
    : undefined;
  ...
  const findMasterOptions = useCallback(
  ...
  const masterCity: any[] = useMemo(
    () => findMasterOptions(["city"]),
    [findMasterOptions],
  );
  const cityOptions = useMemo(
    () => masterCity.map((o) => ({ value: o.value, label: o.label })),
    [masterCity],
  );

  const budgetMaster: any[] = useMemo(
    () =>
      findMasterOptions([
        "price range",
        "price_range",
  ...
  const budgetOptions = useMemo(
    () => budgetMaster.map((o) => ({ value: o.value, label: o.label })),
    [budgetMaster],
  );

  const propertyTypesMaster: any[] = useMemo(
    () =>
      findMasterOptions([
        "property type",
        "property_type",
  ...
  const propertySubtypesMaster: any[] = useMemo(
    () =>
      findMasterOptions([
        "property subtype",
        "property_subtype",
  ...
  const propertySubtypeOptions = useMemo(
    () =>
      propertySubtypesMaster.map((o) => ({
        value: o.value || o.label,
        label: o.label || o.value,
      })),
    [propertySubtypesMaster],
  );

  const unitTypesMaster: any[] = useMemo(
    () =>
      findMasterOptions(["unit type", "unit_type", "unit", "bhk", "bedrooms"]),
    [findMasterOptions],
  );
  ...
  const bedroomsMaster: any[] = useMemo(
    () =>
      findMasterOptions([
        "bedrooms",
        "bhk",
  ...
  useEffect(() => {
    ...
    const rawTx =
      ...
      qp.get("type") ||
      "buy";
    const transactionFromUrl = rawTx.toLowerCase().includes("rent")
      ? "rent"
      : "buy";
  ...
  const loadPropertiesFromSearch = useCallback(async () => {
    ...
    try {
      ...
      const hasAdvanced = Boolean(
        ...
        qp.get("bathrooms") ||
        qp.get("bedrooms") ||
        qp.get("property_subtype") ||
        qp.get("propertySubtype") ||
        qp.get("sort"),
      );
      ...
      const normalizeResponse = (resp: any): any[] => {
        ...
        if (resp?.properties && Array.isArray(resp.properties))
      ...
      const buildAdvancedParams = () => {
        ...
        if (qp.get("unitTypes")) {
          ...
          unitTypes = [qp.get("unitType") || qp.get("unit_type")!].filter(
            Boolean,
          ) as string[];
        }
        if (unitTypes.length) {
        ...
        if (budgetMin) {
          params.budget_min = Number(budgetMin);
          params.minPrice = Number(budgetMin);
        }
        if (budgetMax) {
          params.budget_max = Number(budgetMax);
          params.maxPrice = Number(budgetMax);
        }
        ...
        if (qp.get("min_rating"))
          params.minRating = Number(qp.get("min_rating"));
      ...
      const hadPropertyTypeInUrl = Boolean(
      ...
      const rawTx =
        ...
        "";
      const isRent =
        rawTx.toLowerCase().includes("rent") || transactionType === "rent";
      ...
      if (hasAdvanced) {
        ...
        if (!list.length) {
          ...
          delete relaxed.propertySubtype;
          delete relaxed.property_subtype;
          delete relaxed.unitTypes;
          delete relaxed.unitType;
          delete relaxed.unit_type;
          delete relaxed.furnishing;
          delete relaxed.possession;
          delete relaxed.parking;
          delete relaxed.minRating;
          delete relaxed.floor_min;
          delete relaxed.floor_max;
          delete relaxed.bathrooms;
          if ("bedrooms" in relaxed && !Number(relaxed.bedrooms))
          ...
          try {
            ...
            console.warn(
              "Relaxed advanced failed, will try simple list. Error:",
              e2,
            );
            list = [];
          }
        ...
        if (!list.length) {
          try {
            ...
            console.warn(
              "PublicgetProperties failed after advanced attempts",
              e3,
            );
            list = [];
          }
        ...
        if (!list.length && hadPropertyTypeInUrl) {
          try {
            ...
            delete noPT.propertyType;
            delete noPT.property_type;
      ...
      const allTagsBulk = await propertyTagsAPI
        .getBulk(list.map((p: any) => p.id))
      ...
      const transformedProperties = await Promise.all(
        list.map(async (p: any, index: number) => {
          ...
          const propertyData: Property = {
            ...
            title:
              p.title ||
              `${p.unit_type || ""} ${p.property_type_name || ""}`.trim() ||
              `Property ${p.id}`,
            price:
            ...
            status: p.status || "",
            images,
            location: `${p.location_name || p.location || ""}`.replace(
              /\s*,\s*$/,
              "",
            ),
            ...
            furnishing:
              p.furnishing_status ||
              ["Fully Furnished", "Semi Furnished", "Unfurnished"][index % 3],
            possession:
              p.possession_status ||
              ["Ready to Move", "Under Construction"][index % 2],
            available_from:
              p.available_from ||
              p.available_date ||
              p.possession_date ||
            ...
            postedDate: p.created_at
              ...
              : `2025-01-${String(Math.floor(Math.random() * 15) + 1).padStart(2, "0")}`,
            views: viewCounts.total_views || 0,
            total_views: viewCounts.total_views,
            aiScore: p.ai_score || Math.floor(Math.random() * 30) + 70,
            priceGrowth:
              p.price_growth || `+${(Math.random() * 20 + 5).toFixed(1)}%`,
            investmentGrade:
              p.investment_grade ||
              ["A++", "A+", "A", "B+"][Math.floor(Math.random() * 4)],
    ...
    if (!value) {
      ...
      const baseDest =
        transactionType === "rent"
          ? "/properties?transaction=rent&tab=rent"
          : "/properties?transaction=buy";
  ...
  useEffect(() => {
    if (selectedType !== undefined) {
      setSelectedPropertyType((prev) =>
        prev !== (selectedType || "") ? selectedType || "" : prev,
      );
    }
  }, [selectedType]);
  ...
  const filteredProperties = allProperties.filter((property) => {
    ...
    const propSubtype = String(
      (property as any)._raw?.property_subtype_name ||
        (property as any)._raw?.property_subtype ||
    ...
    const q = String(searchQuery || "")
      .trim()
      .toLowerCase();
    const selLoc = String(selectedLocation || "")
      .trim()
      .toLowerCase();
    const selType = String(selectedType || "")
      .trim()
      .toLowerCase();
    const selSubtype = String(selectedPropertySubtype || "")
      .trim()
      .toLowerCase();
    const selBudget = String(selectedBudget || "").trim();
    const selBedrooms = String(selectedBedrooms || "").trim();
    const selUnitType = String(selectedUnitType || "")
      .trim()
      .toLowerCase();
    ...
    const matchesLocation =
      ...
      (localities.length > 0 &&
        localities.some((l) => propLocation.includes(l.toLowerCase())));
    ...
    const matchesBedrooms =
      selBedrooms === "" ||
      selBedrooms === "Any" ||
      (Number(selBedrooms) > 0
        ? property.bedrooms === Number(selBedrooms)
        : selBedrooms.toString().toLowerCase().includes("1")
          ? property.bedrooms === 1
          : true);
    ...
    const matchesUnitType =
      ...
      (Number.isFinite(Number(selUnitType)) &&
        Number(property.bedrooms) === Number(selUnitType));
    ...
    if (
      possessionFilter &&
      possessionFilter !== "" &&
      !String(property.possession || "")
        .toLowerCase()
        .includes(String(possessionFilter).toLowerCase())
    ) {
      return false;
    }
  ...
  const getPropertyPrice = (p: Property) => {
    const pr =
      ...
      p.monthly_rent ||
      p.expected_rent ||
      (p as any)._raw?.monthly_rent ||
      (p as any)._raw?.expected_rent ||
      (p as any)._raw?.price ||
      (p as any)._raw?.budget ||
      0;
    return Number(pr) || 0;
  };
  ...
  const getPostedTimestamp = (p: Property) => {
    const val =
      p.postedDate || (p as any)._raw?.created_at || (p as any).created_at;
    if (!val) return 0;
  ...
  const getAvailableTimestamp = (p: Property) => {
    const val =
      p.available_from ||
      (p as any).availableFrom ||
      (p as any)._raw?.available_from ||
      (p as any)._raw?.availableFrom ||
      p.possession ||
      (p as any)._raw?.possession_status;
  ...
  const handleNavigateToProperty = async (property: Property) => {
    const { isLocked } = recordAndCheckGuestPropertyLimit(
      property.id,
      user,
      systemSettings,
    );
    if (isLocked) {
      const isRental = Boolean(
        ...
        (property.transaction_type &&
          String(property.transaction_type).toLowerCase() === "rent") ||
        (property.purpose &&
      ...
      const redirectUrl = property.slug
        ? `/${pathPrefix}/${encodeURIComponent(String(property.slug))}`
        : "/properties";
    ...
    if (viewedProperties.has(id)) {
      const mergedQs = preserveAndAddToken(
        location.search,
        filterParamKey || "fltcnt",
        filterTokenFromUrl,
      );
    ...
    if (!finalToken) {
      try {
        const createRes = await propertiesAPI.createFilterContext({
          filters: inferredFilters,
        });
        if (createRes) {
          const idFromRes =
            (createRes as any).id || (createRes as any).filterId || null;
          if (idFromRes) finalToken = String(idFromRes);
        }
      } catch (err) {
        console.warn(
          "createFilterContext failed (continuing without token):",
          err,
        );
      }
    }
    ...
    try {
      await propertiesAPI.sendPropertyEvent(
        ...
        {
          slug,
          filterToken: finalToken || undefined,
          filterParamKey: finalParamKey,
        },
      );
  ...
  if (currentPropertyView) {
    const isRental = Boolean(
      ...
      (currentPropertyView.listing_type &&
        String(currentPropertyView.listing_type).toLowerCase() === "rent") ||
      (currentPropertyView.transaction_type &&
        String(currentPropertyView.transaction_type).toLowerCase() ===
          "rent") ||
      (currentPropertyView.purpose &&
        String(currentPropertyView.purpose).toLowerCase() === "rent") ||
      (currentPropertyView.raw?.purpose &&
        String(currentPropertyView.raw.purpose).toLowerCase() === "rent") ||
      (currentPropertyView.raw?.transaction_type &&
        String(currentPropertyView.raw.transaction_type).toLowerCase() ===
          "rent") ||
      (currentPropertyView.raw?.listing_type &&
        String(currentPropertyView.raw.listing_type).toLowerCase() ===
          "rent") ||
        ...
        */}
        <div
          ...
          style={{
            zIndex: 1,
            background:
              "linear-gradient(to bottom, rgba(11, 56, 86, 0.35) 0%, rgba(11, 56, 86, 0.25) 40%, rgba(11, 56, 86, 0.45) 70%, rgba(11, 56, 86, 0.60) 100%)",
          }}
        />
        ...
        >
          <div className="text-center">
            ...
            <h1
              ...
              style={{
                textShadow:
                  "0 2px 12px rgba(0,0,0,0.75), 0 1px 3px rgba(0,0,0,0.9)",
              }}
            >
              Explore <span className="text-[#E6761D]">Premium Properties</span>
            </h1>
            ...
            <p
              ...
              style={{
                textShadow:
                  "0 1px 8px rgba(0,0,0,0.8), 0 1px 2px rgba(0,0,0,0.9)",
              }}
            >
              Welcome to{" "}
              <span className="font-semibold text-white">ResaleExpert</span> —
              India's premier real estate ecosystem connecting buyers, sellers,
              and landlords with verified luxury properties, transparent
            ...
            <div className="grid grid-cols-1 gap-1 md:gap-2 mb-4 items-center justify-center text-center">
              ...
              <div className="w-full grid justify-center md:w-auto">
                {masterLoading ? (
                  <div className="text-sm text-white/80 px-3 py-1">
                    Loading types...
                  </div>
                ) : (
                  <div
                    className="relative ml-2 md:ml-0 max-w-full overflow-x-auto [-webkit-overflow-scrolling:touch] [scrollbar-width:none] [-ms-overflow-style:none] px-1"
            ...
            <form
              ...
              className="bg-white/15 text-white backdrop-blur-md rounded-2xl p-2 shadow-xl max-w-4xl mx-auto border border-white/20"
            >
                ...
                <div className="relative md:col-span-3 flex items-center gap-2 bg-[#0b3856]/70 backdrop-blur-sm border border-white/20 rounded-xl w-full max-w-[700px] mx-auto">
                  <div className="relative flex-1">
                    ...
                    {showSuggestions && suggestions.length > 0 && (
                      <ul
                        role="listbox"
                        className="absolute left-0 right-0 mt-1 max-h-32 lg:max-w-60 overflow-auto bg-[#0b3856] border rounded-lg shadow-lg z-[200] custom-scroll"
                      >
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex flex-wrap items-center justify-between w-full gap-3 sm:gap-4">
              ...
              <div className="flex-shrink-0 min-w-[180px]">
                <h2 className="text-lg sm:text-xl font-bold text-[#0b3856]">
                  All Properties{" "}
                  {!loading && !error && `(${allProperties.length})`}
                </h2>

                {Boolean(
                  selectedLocation || localities.length || selectedBudget,
                ) && (
                  <p className="text-gray-600 text-xs sm:text-sm leading-snug">
                    ...
                    of {allProperties.length} results
                  </p>
                )}
              </div>
                    ...
                    <div className="flex items-start gap-2 px-3 py-2 bg-yellow-50 border border-yellow-200 rounded-lg shadow-sm text-xs sm:text-sm max-w-full">
                      ...
                      <div className="flex-1 leading-snug">
                        ...
                        <div className="text-gray-700">
                          ...
                          public properties. {selectedLocation || "Top areas"}{" "}
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-3 flex-shrink-0">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className={`p-2 rounded-lg w-9 h-9 flex items-center justify-center transition ${
                      viewMode === "grid"
          ...
          <div className="flex items-end justify-end gap-3 mt-2">
            ...
            <div className="relative">
              ...
              {showFilters && (
                ...
                >
                  <h3 className="text-lg font-semibold text-[#0b3856] mb-4">
                    Advanced Filters
                  </h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    ...
                    <div>
                      ...
                      >
                        <option value="">
                          {masterLoading ? "Loading budgets..." : "Any Budget"}
                        </option>
                        {budgetOptions.map((b) => (
                          <option
                            key={b.value || b.label}
                    ...
                    <div>
                      ...
                      >
                        <option value="">
                          {masterLoading ? "Loading types..." : "All Types"}
                        </option>
                    ...
                    <div className="col-span-full flex gap-3 justify-end pt-2">
                      <button
                        onClick={() => {
                          ...
                          if (possessionFilter)
                            params.set("possession", possessionFilter);
                          if (parkingFilter && parkingFilter !== "any")
                            params.set("parking", parkingFilter);
                          if (floorMin !== "")
                            params.set("floor_min", String(floorMin));
                          if (floorMax !== "")
                            params.set("floor_max", String(floorMax));
                          if (bathroomsFilter !== "")
                            params.set("bathrooms", String(bathroomsFilter));
                          ...
                          if (selectedUnitType)
                            params.set("unitType", selectedUnitType);
        ...
        {error && !loading && (
          <div className="text-center py-16">
            ...
            <h3 className="text-lg font-bold text-[#0b3856] mb-4">
            ...
            {viewMode === "grid" ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 items-stretch">
                {paginatedProperties.map((property) => {
                  ...
                  const { locationPart, cityPart } =
                    splitLocationCity(property);
                    ...
                    >
                      ...
                      <div className="relative overflow-hidden">
                        <img
                          ...
                          onError={(e) => {
                            e.currentTarget.src = "/property.png";
                          }}
                        ...
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                          <span className="text-white text-xl font-bold opacity-30 select-none">
                            ResaleExpert.in
                          </span>
                        </div>
                      ...
                      <div className="p-3 flex-1 flex flex-col">
                        ...
                        <div className="mb-1.5">
                          ...
                          <div className="text-[10px] text-gray-500">
                            ...
                            {property.area || property.square_feet} sq ft
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1 mb-2">
                          {shownAmenities.map((amenity, i) => (
                            <div
                              key={i}
                              className="flex items-center space-x-0.5 px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded-full text-[10px]"
                            >
                        ...
                        <div className="mt-auto flex items-center space-x-1.5">
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              const phone =
                                property.executiveTo?.phone || "919999999999";
                          ...
                          <button
                            onClick={(e) => {
                              ...
                              const phone =
                                property?.executiveTo?.phone?.replace(
                                  /\D/g,
                                  "",
                                ) || "9637009639";
                              const title =
              ...
              <div className="space-y-3">
                {paginatedProperties.map((property) => {
                  ...
                  const { locationPart, cityPart } =
                    splitLocationCity(property);
                  const parkingCount = Number.isFinite(Number(property.parking))
                  ...
                  const pricePerSqFt =
                    property.price && (property.area || property.square_feet)
                      ? Math.round(
                          property.price /
                            (Number(property.area || property.square_feet) ||
                              1),
                        )
                      : null;
                        ...
                        <div className="flex-1 flex flex-col justify-between px-2 py-2 sm:px-4 sm:py-3 min-w-0 gap-1 sm:gap-2 overflow-hidden">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 sm:gap-2">
                            <div className="bg-gray-50 rounded-lg p-1.5 sm:p-2">
                              <div className="text-[9px] sm:text-[10px] text-gray-400 uppercase tracking-wide mb-0.5">
                                Price
                              </div>
                            ...
                            <div className="bg-gray-50 rounded-lg p-1.5 sm:p-2">
                              ...
                              <div className="text-xs sm:text-sm font-medium text-blue-700">
                                {property.area || property.square_feet}{" "}
                                <span className="text-[9px]">sq.ft</span>
                              </div>
                              <div className="hidden sm:block text-[10px] text-gray-400">
                          ...
                          <div className="flex sm:hidden items-center gap-1.5 text-[10px] text-gray-500">
                            <span className="flex items-center gap-0.5">
                              <Bed size={10} className="text-orange-400" />
                            ...
                            <div className="hidden sm:flex items-center gap-1.5 flex-wrap">
                              {showAmenities.map(
                                (amenity: string, idx: number) => (
                                  <span
                                    key={idx}
                                    className="text-[10px] text-blue-600 bg-blue-50 rounded-full px-2 py-0.5 truncate max-w-[90px]"
                                  >
                                    {amenity}
                                  </span>
                                ),
                  ...
                  {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-4 py-2 rounded-lg ${currentPage === pageNumber ? "bg-blue-600 text-white" : "border border-gray-300 hover:bg-gray-50"}`}
                      >
                        {pageNumber}
                      </button>
                    );
        ...
        {!loading &&
          ...
          allProperties.length > 0 && (
            <div className="text-center py-16">
              <Home className="mx-auto text-gray-300 mb-6" size={64} />
              <h3 className="text-2xl font-bold text-[#0b3856] mb-4">
                No Properties Found
              </h3>
