...
export function MastersAdmin({ master, onChanged }: Props) {
  ...
  const [modalItem, setModalItem] = useState<{ mode: 'add' | 'edit'; tab: TabKey; id?: string } | null>(null);
  const [moduleBusy, setModuleBusy] = useState(false);
  const [moduleFeedback, setModuleFeedback] = useState<{ type: 'success' | 'error'; text: string } | null>(null);
  const [editingSequence, setEditingSequence] = useState<SequenceStep | null>(null);
  ...
  const nameFor = useMemo(() => {
    const match = (itemCode?: string | null, itemName?: string | null, target?: string | null) => {
    ...
    return {
      ...
      status: (code?: string | null, entity?: string | null) =>
        master.statuses.find((s) => match(s?.code, s?.name, code) && (!entity || s?.entity_code === entity))?.name ??
        DEFAULT_RULE_STATUSES.find((s) => match(s?.code, s?.name, code))?.name ??
        code ??
        '—',
      outcome: (code?: string | null) =>
        master.outcomes.find((o) => match(o?.code, o?.name, code))?.name ??
        DEFAULT_RULE_OUTCOMES.find((o) => match(o?.code, o?.name, code))?.name ??
        code ??
        '—',
      action: (code?: string | null) =>
        master.nextActions.find((a) => match(a?.code, a?.name, code))?.name ?? code ?? '—',
  ...
  const filteredRules = useMemo(() => {
    ...
    if (query) {
      ...
      r = r.filter((x) => {
        const ruleId = (x.rule_id || (x as any).name || x.id || '').toLowerCase();
        const typeName = (nameFor.type(x.follow_up_type_code) || '').toLowerCase();
        const stageName = (nameFor.stage(x.current_stage_code, x.entity_code) || '').toLowerCase();
        const statusName = (nameFor.status(x.current_status_code, x.entity_code) || '').toLowerCase();
        const outcomeName = (x.outcome_code || '').toLowerCase();
        const remarkText = (x.remark || (x as any).auto_remark_template || '').toLowerCase();
        return (
          ruleId.includes(q) ||
          typeName.includes(q) ||
  ...
  const filteredSequenceNames = useMemo(() => {
    ...
    if (query) {
      ...
      list = list.filter((s) => {
        ...
        const action = (nameFor.action(s.action_code) || s.action_code || '').toLowerCase();
        const type = (nameFor.type(s.follow_up_type_code) || s.follow_up_type_code || '').toLowerCase();
        const priority = (s.priority_code || '').toLowerCase();
        const status = (s.next_status_code ? nameFor.status(s.next_status_code) : '').toLowerCase();
        return name.includes(q) || action.includes(q) || type.includes(q) || priority.includes(q) || status.includes(q);
      });
    }
  ...
  const filteredFollowUpTypes = useMemo(() => {
    ...
    if (entityFilter !== 'All') {
      list = list.filter((t) => !(t as any).entity_code || (t as any).entity_code === entityFilter);
    }
    if (query) {
      const q = query.toLowerCase();
  ...
  const filteredReasons = useMemo(() => {
    ...
    if (query) {
      const q = query.toLowerCase();
      list = list.filter((r) => (r.name || '').toLowerCase().includes(q) || (r.code || '').toLowerCase().includes(q) || ((r as any).outcome_code || '').toLowerCase().includes(q));
    }
    return list;
  ...
  const filteredPriorities = useMemo(() => {
    ...
    if (query) {
      const q = query.toLowerCase();
      list = list.filter((p) => (p.name || '').toLowerCase().includes(q) || (p.code || '').toLowerCase().includes(q) || String(p.level).includes(q));
    }
    return list;
  }, [master.priorities, query]);

  const sequenceNames = useMemo(() => {
    const names = new Set(master.sequences.map((s) => s.sequence_name).filter(Boolean));
    return Array.from(names).sort();
  }, [master.sequences]);
  ...
  // Common entity pills for filter
    ...
    <div className="admin">
      ...
      <div className="admin-tabs-wrapper">
        <div className="admin-tabs">
          {tabs.map((t) => (
            ...
            >
              {t.label}
              {t.key === 'rules' && <span className="tab-badge">{master.rules.length}</span>}
              {t.key === 'sequences' && <span className="tab-badge">{sequenceNames.length}</span>}
            </button>
          ))}
        </div>
      ...
      {tab === 'rules' && (
        <div className="admin-controls-stack">
          {/* Row 1: Entity Filter Pills, Search Box, and Type Pills */}
          ...
          {/* Row 2: Show remarks, Add rule, Import & Export Buttons */}
      ...
      {/* Control Area for Simple Master tabs (Entities, Types, Stages, Statuses, Outcomes, Reasons, Actions, Priorities) */}
      ...
      {tab === 'rules' && (
        <div className="rules-card-container">
          ...
          {filteredRules.length > 50 && (
            <button className="rules-show-more-btn" onClick={() => setShowAll((s) => !s)}>
                ...
                <div className="seq-card-header">
                  <div className="seq-header-left">
                    ...
                    <div className="seq-title-group">
                      ...
                      <span>
                        {steps.length} steps · {activeCount} active · {totalSpan}d total span
                      </span>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    {terminalCount > 0 && (
                      <span className="terminal-pill-badge">{terminalCount} TERMINAL</span>
                    )}
                    ...
                    return (
                      <div className="timeline-step-row" key={s.id}>
                        <div className="timeline-rail">
                          <div className={`timeline-circle ${s.terminal_step ? 'terminal' : ''}`}>
                            {s.step}
                          </div>
                        ...
                        <div className="timeline-step-body">
                          <div className="step-delay-badges">
                            ...
                            {s.terminal_step && (
                              <span className="step-terminal-pill">TERMINAL</span>
                            )}
                          </div>

                          <div className="step-main-line">
                            <div className="step-action-title-group">
                              <span className="step-action-name">{nameFor.action(s.action_code)}</span>
                              <span className="step-type-pill">{nameFor.type(s.follow_up_type_code)}</span>
                              <span className={`step-priority-pill ${(s.priority_code || '').toLowerCase()}`}>
                                {s.priority_code}
                              </span>
                            </div>
                          ...
                          {(s.next_status_code || s.reason_code) && (
                            <div className="step-subnote">
                              ...
                              {s.reason_code && ` · ${nameFor.reason(s.reason_code)}`}
                            </div>
                          )}
                        </div>
                      </div>
              ...
              {filteredEntities.map((e) => {
                ...
                return (
                  <tr key={e.id}>
                    ...
                    <td>
                      <span className="order-slate-text">Order {e.display_order}</span>
                    </td>
                    <td>
                      <button
                        className={`active-pill-badge ${e.is_active ? 'active' : 'inactive'}`}
                        onClick={() => void handleToggleFlag(e.id, 'is_active', e.is_active)}
                      >
                    ...
                    <td>
                      <div className="master-row-actions">
                        ...
                        <button className="del" onClick={() => void handleDelete(e.id)} title="Delete">
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </td>
                  ...
                  <tr key={t.id}>
                    ...
                    <td>
                      {(t as any).entity_code ? (
                        <span className="entity-pill-badge">{nameFor.entity((t as any).entity_code)}</span>
                      ) : (
                        <span className="order-slate-text">All</span>
                      )}
                    </td>
                    <td>
                      <span className="order-slate-text">Order {t.display_order}</span>
                    </td>
                    <td>
                      <button
                        className={`active-pill-badge ${t.is_active ? 'active' : 'inactive'}`}
                        onClick={() => void handleToggleFlag(t.id, 'is_active', t.is_active)}
                      >
                    ...
                    <td>
                      <div className="master-row-actions">
                        ...
                        <button className="del" onClick={() => void handleDelete(t.id)} title="Delete">
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </td>
              ...
              {filteredStages.map((s) => {
                ...
                return (
                  <tr key={s.id}>
                    ...
                    <td>
                      <span className="entity-pill-badge">{nameFor.entity(s.entity_code)}</span>
                    </td>
                    <td>
                      <span className="order-slate-text">Order {s.display_order}</span>
                    </td>
                    <td>
                      <button
                        className={`active-pill-badge ${s.is_active ? 'active' : 'inactive'}`}
                        onClick={() => void handleToggleFlag(s.id, 'is_active', s.is_active)}
                      >
                    ...
                    <td>
                      <div className="master-row-actions">
                        ...
                        <button className="del" onClick={() => void handleDelete(s.id)} title="Delete">
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </td>
              ...
              {filteredOutcomes.map((o) => {
                ...
                return (
                  <tr key={o.id}>
                    ...
                    <td>
                      <span className="entity-pill-badge">{nameFor.type(o.follow_up_type_code)}</span>
                    </td>
                    <td>
                      <span className="order-slate-text">Order {o.display_order}</span>
                    </td>
                    <td>
                      <button
                        className={`active-pill-badge ${o.is_active ? 'active' : 'inactive'}`}
                        onClick={() => void handleToggleFlag(o.id, 'is_active', o.is_active)}
                      >
                    ...
                    <td>
                      <div className="master-row-actions">
                        ...
                        <button className="del" onClick={() => void handleDelete(o.id)} title="Delete">
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </td>
              ...
              {filteredReasons.map((r) => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td>
                      <span className="order-slate-text">Order {r.display_order}</span>
                    </td>
                    <td>
                      <button
                        className={`active-pill-badge ${r.is_active ? 'active' : 'inactive'}`}
                        onClick={() => void handleToggleFlag(r.id, 'is_active', r.is_active)}
                      >
                    ...
                    <td>
                      <div className="master-row-actions">
                        ...
                        <button className="del" onClick={() => void handleDelete(r.id)} title="Delete">
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </td>
              ...
              {filteredNextActions.map((a) => {
                ...
                return (
                  <tr key={a.id}>
                    ...
                    <td>
                      <span className="order-slate-text">Order {a.display_order}</span>
                    </td>
                    <td>
                      <button
                        className={`active-pill-badge ${a.is_active ? 'active' : 'inactive'}`}
                        onClick={() => void handleToggleFlag(a.id, 'is_active', a.is_active)}
                      >
                    ...
                    <td>
                      <div className="master-row-actions">
                        ...
                        <button className="del" onClick={() => void handleDelete(a.id)} title="Delete">
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </td>
              ...
              {filteredPriorities.map((p) => {
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <td>
                      <span className="order-slate-text">Level {p.level}</span>
                    </td>
                    <td>
                      <button
                        className={`active-pill-badge ${p.is_active ? 'active' : 'inactive'}`}
                        onClick={() => void handleToggleFlag(p.id, 'is_active', p.is_active)}
                      >
                    ...
                    <td>
                      <div className="master-row-actions">
                        ...
                        <button className="del" onClick={() => void handleDelete(p.id)} title="Delete">
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </td>
...
function RuleForm({
  ...
  // 1. Entities from master.entities
  // 1. Entities from master.entities (deduplicated)
  ...
  // 2. Types from master.followUpTypes (filtered by entity if mapped, deduplicated by code)
  const availableTypes = useMemo(() => {
    const list = master.followUpTypes.filter(
      (t) => (!form.entity_code || !(t as any).entity_code || (t as any).entity_code === form.entity_code) && t.is_active !== false
    );
    const source = list.length > 0 ? list : master.followUpTypes.length > 0 ? master.followUpTypes : DEFAULT_RULE_FOLLOW_UP_TYPES;
    const map = new Map<string, { code: string; name: string }>();
    source.forEach((t) => {
  ...
  // 3. Stages from master.stages (cascading linked to form.entity_code, always deduplicated)
  const availableStages = useMemo(() => {
    const list = master.stages.filter(
      (s) => (!form.entity_code || s.entity_code === form.entity_code) && s.is_active !== false
    );
    const source = list.length > 0 ? list : master.stages.length > 0 ? master.stages : DEFAULT_RULE_STAGES;
    const map = new Map<string, { code: string; name: string }>();
    source.forEach((s) => {
  ...
  // 4. Statuses from master.statuses (cascading linked to form.entity_code, always deduplicated)
  const availableStatuses = useMemo(() => {
    const list = master.statuses.filter(
      (s) => (!form.entity_code || s.entity_code === form.entity_code) && s.is_active !== false
    );
    const source = list.length > 0 ? list : master.statuses.length > 0 ? master.statuses : DEFAULT_RULE_STATUSES;
    const map = new Map<string, { code: string; name: string }>();
    source.forEach((s) => {
      const key = (s.code || s.name).toUpperCase();
      if (!map.has(key)) {
  ...
  // 5. Outcomes from master.outcomes (cascading linked to form.follow_up_type_code & form.entity_code, always deduplicated)
  const availableOutcomes = useMemo(() => {
    const list = master.outcomes.filter(
      (o) =>
        (!form.follow_up_type_code || o.follow_up_type_code === form.follow_up_type_code) &&
        (!form.entity_code || !(o as any).entity_code || (o as any).entity_code === form.entity_code) &&
        o.is_active !== false
    );
    const source = list.length > 0 ? list : master.outcomes.length > 0 ? master.outcomes : DEFAULT_RULE_OUTCOMES;
    const map = new Map<string, { code: string; name: string }>();
    source.forEach((o) => {
  ...
  // 6. Reasons from master.reasons (filtered by outcome if outcome_code set, always deduplicated)
  const availableReasons = useMemo(() => {
    const list = master.reasons.filter(
      (r) => (!form.outcome_code || !(r as any).outcome_code || (r as any).outcome_code === form.outcome_code) && r.is_active !== false
    );
  ...
  // 7. Next Actions from master.nextActions (always deduplicated)
  const availableNextActions = useMemo(() => {
    const map = new Map<string, { code: string; name: string }>();
    master.nextActions.filter((a) => a.is_active !== false).forEach((a) => {
  ...
  // 8. Priorities from master.priorities (always deduplicated)
  const availablePriorities = useMemo(() => {
    const map = new Map<string, { code: string; name: string }>();
    master.priorities.filter((p) => p.is_active !== false).forEach((p) => {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const payload: any = {
      ...
      auto_remark_template: form.remark || (form as any).auto_remark_template || null,
      ...
      <div className="modal rule-form-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div className="modal-body">
            ...
            <div className="rule-form-section">
              ...
              <div className="form-grid">
                <div className="field">
                  <label>Rule ID <span className="req-star">*</span></label>
                  <input
                    value={form.rule_id ?? ''}
                ...
                <div className="field">
                  <label>Entity <span className="req-star">*</span></label>
                ...
                <div className="field">
                  <label>Follow-up type <span className="req-star">*</span></label>
                  <div className="select-wrap">
                    <select
                ...
                <div className="field">
                  ...
                  <div className="select-wrap">
                    <select
                      ...
                      onChange={(e) => update('current_stage_code', e.target.value)}
                    >
                ...
                <div className="field">
                  <label>Current status <span className="req-star">*</span></label>
                  <div className="select-wrap">
                    <select
                      value={form.current_status_code ?? ''}
                      onChange={(e) => update('current_status_code', e.target.value)}
                    >
            ...
            <div className="rule-form-section">
              ...
              <div className="form-grid">
                <div className="field">
                  <label>Next stage <span className="req-star">*</span></label>
                  <div className="select-wrap">
                    <select
                      value={form.next_stage_code ?? ''}
                      onChange={(e) => update('next_stage_code', e.target.value)}
                    >
                      <option value="">Select Stage...</option>
                      {availableStages.map((s) => (
                        <option key={s.code} value={s.code}>{s.name}</option>
                ...
                <div className="field">
                  <label>Next status <span className="req-star">*</span></label>
                  <div className="select-wrap">
                    <select
                      value={form.next_status_code ?? ''}
                      onChange={(e) => update('next_status_code', e.target.value)}
                    >
                      <option value="">Select Status...</option>
                      {availableStatuses.map((s) => (
                        <option key={s.code} value={s.code}>{s.name}</option>
                ...
                <div className="field">
                  <label>Next action <span className="req-star">*</span></label>
                  <div className="select-wrap">
                    <select
                      value={form.next_action_code ?? ''}
                      onChange={(e) => update('next_action_code', e.target.value)}
                    >
                ...
                <div className="field">
                  ...
                  <div className="select-wrap">
                    <select
                      ...
                      onChange={(e) => update('next_follow_up_type_code', e.target.value ? e.target.value : null)}
                    >
              ...
              <div className="form-grid">
                <div className="field">
                  ...
                  <input
                    ...
                    onChange={(e) => update('default_days', Number(e.target.value))}
                    min={0}
                  />
                </div>
                ...
                <div className="field">
                  ...
                  <div className="select-wrap">
                    <select
                      ...
                      onChange={(e) => update('sequence_name', e.target.value ? e.target.value : null)}
                    >
              ...
              <div className="flags-box">
                ...
                <label className="custom-form-checkbox">
                  <input
                    ...
                    onChange={(e) => update('require_follow_up', e.target.checked)}
                  />
            ...
            <div className="rule-form-section">
              ...
              <div className="automation-subtext">
                When this rule fires, automatically send messages via the selected channels. Placeholders:{' '}
          ...
          <div className="modal-footer">
            <button type="button" className="btn-white-secondary" onClick={onClose}>Cancel</button>
            <button type="submit" className="btn-orange-primary" disabled={busy}>
...
function SequenceForm({
  ...
  // Deduplicate follow-up types for dropdown
  const availableTypes = useMemo(() => {
  ...
  // Deduplicate actions for dropdown
  ...
  // Deduplicate priorities for dropdown
  const availablePriorities = useMemo(() => {
    const map = new Map<string, { code: string; name: string }>();
    master.priorities.filter((p) => p.is_active !== false).forEach((p) => {
      const key = (p.code || p.name).toUpperCase();
  ...
  // Deduplicate statuses for dropdown
  const availableStatuses = useMemo(() => {
    const map = new Map<string, { code: string; name: string }>();
    master.statuses.filter((s) => s.is_active !== false).forEach((s) => {
      const key = (s.code || s.name).toUpperCase();
      if (!map.has(key)) {
        map.set(key, { code: s.code, name: s.name });
      }
  ...
  // Deduplicate reasons for dropdown
  const availableReasons = useMemo(() => {
    const map = new Map<string, { code: string; name: string }>();
    master.reasons.filter((r) => r.is_active !== false).forEach((r) => {
      const key = (r.code || r.name).toUpperCase();
      if (!map.has(key)) {
        map.set(key, { code: r.code, name: r.name });
      }
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div className="modal-body">
            ...
            <div className="form-grid">
              <div className="field">
                <label>Sequence name <span className="req-star">*</span></label>
                <input
                  value={form.sequence_name ?? ''}
              ...
              <div className="field">
                <label>Step <span className="req-star">*</span></label>
                <input
                  type="number"
                  value={form.step ?? 1}
              ...
              <div className="field">
                ...
                <input
                  ...
                  onChange={(e) => update('after_hours', Number(e.target.value))}
                  min={0}
                />
              </div>

              <div className="field">
                <label>Action <span className="req-star">*</span></label>
                <div className="select-wrap">
                  <select
              ...
              <div className="field">
                ...
                <div className="select-wrap">
                  <select
                    value={form.follow_up_type_code ?? ''}
                    onChange={(e) => update('follow_up_type_code', e.target.value)}
                  >
                    <option value="">Select Type...</option>
                    {availableTypes.map((t) => (
                      <option key={t.code} value={t.code}>{t.name}</option>
              ...
              <div className="field">
                ...
                <div className="select-wrap">
                  <select
                    value={form.next_status_code ?? ''}
                    onChange={(e) => update('next_status_code', e.target.value ? e.target.value : null)}
                  >
            ...
            <div className="modal-checkbox-card grid-2" style={{ marginTop: '12px' }}>
...
function MasterItemModal({
  ...
  const [form, setForm] = useState<Record<string, unknown>>(() => {
    if (currentRecord) {
      ...
      if (!rec.icon && rec.code) {
        ...
        if (fallback[rec.code.toUpperCase()]) rec.icon = fallback[rec.code.toUpperCase()];
      }
      return rec;
    }
    ...
    if (tab === 'entities') {
      ...
      initial.entity_code = entityFilter !== 'All' ? entityFilter : master.entities[0]?.code ?? 'LEAD';
      initial.icon = 'Phone';
      ...
      initial.follow_up_type_code = typeFilter !== 'All' ? typeFilter : master.followUpTypes[0]?.code ?? 'CALL';
  ...
  const distinctEntities = useMemo(() => {
    const map = new Map<string, { code: string; name: string }>();
    master.entities.filter((e) => e.is_active !== false).forEach((e) => {
      const key = (e.code || e.name).toUpperCase();
      if (!map.has(key)) {
        map.set(key, { code: e.code, name: e.name });
      }
  ...
  const distinctFollowUpTypes = useMemo(() => {
    const map = new Map<string, { code: string; name: string }>();
    master.followUpTypes.filter((t) => t.is_active !== false).forEach((t) => {
      const key = (t.code || t.name).toUpperCase();
      if (!map.has(key)) {
        map.set(key, { code: t.code, name: t.name });
      }
  ...
  const distinctOutcomes = useMemo(() => {
    const map = new Map<string, { code: string; name: string }>();
    master.outcomes.filter((o) => o.is_active !== false).forEach((o) => {
      const key = (o.code || o.name).toUpperCase();
      if (!map.has(key)) {
        map.set(key, { code: o.code, name: o.name });
      }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setBusy(true);
    setError(null);
    const codeStr = (form.code as string)?.trim() || nameStr.toUpperCase().replace(/\s+/g, '_');
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-header">
          ...
          <div>
            <h2>{mode === 'edit' ? `Edit ${singularLabel}` : `Add ${singularLabel}`}</h2>
            <p>{mode === 'edit' ? `Update all fields for this ${singularLower}` : `Add a new ${singularLower}`}</p>
          </div>
          <button className="close-button" onClick={onClose}><X size={18} /></button>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
              ...
              <>
                <div className="form-grid">
                  ...
                  <div className="field">
                    ...
                    <div className="select-wrap">
                      <select
                        value={(form.icon as string) || (form.icon_name as string) || 'Phone'}
                        onChange={(e) => {
                ...
                <div className="modal-checkbox-card grid-3">
                  <label className="custom-checkbox-label">
                    <input
                      ...
                      onChange={(e) => update('requires_outcome', e.target.checked)}
                    />
                  ...
                  <label className="custom-checkbox-label">
                    <input
                      ...
                      onChange={(e) => update('requires_date', e.target.checked)}
                    />
                  ...
                  <label className="custom-checkbox-label">
                    <input
                      ...
                      onChange={(e) => update('requires_time', e.target.checked)}
                    />
                  ...
                  <label className="custom-checkbox-label">
                    <input
                      ...
                      onChange={(e) => update('requires_project', e.target.checked)}
                    />
                  ...
                  <label className="custom-checkbox-label">
                    <input
                      ...
                      onChange={(e) => update('requires_participants', e.target.checked)}
                    />
                  ...
                  <label className="custom-checkbox-label">
                    <input
                      ...
                      onChange={(e) => update('requires_template', e.target.checked)}
                    />
                  ...
                  <label className="custom-checkbox-label">
                    <input
                      type="checkbox"
                      checked={((form.creates_next_task ?? form.auto_create_next_task) as boolean) ?? true}
                      onChange={(e) => {
          ...
          <div className="modal-footer">
            <button type="button" className="btn-white-secondary" onClick={onClose}>Cancel</button>
            <button type="submit" className="btn-orange-primary" disabled={busy}>
              <Check size={16} /> {busy ? 'Saving...' : `${mode === 'edit' ? 'Save' : 'Add'} ${singularLower}`}
            </button>
          </div>
        </form>
      </div>
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" style={{ maxWidth: '520px' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '16px', padding: '20px' }}>
          {error && (
            <div style={{ color: '#dc2626', background: '#fef2f2', padding: '10px 14px', borderRadius: '8px', fontSize: '13px', border: '1px solid #fecaca' }}>
              {error}
            </div>
          )}

          {/* Section 1: Download Sample Template */}
          ...
          <div>
            ...
            >
              <Import size={28} style={{ color: file ? '#16a34a' : '#94a3b8', margin: '0 auto 8px' }} />
              {file ? (
                <div>
                  <strong style={{ display: 'block', color: '#15803d', fontSize: '14px' }}>{file.name}</strong>
                  <span style={{ fontSize: '12px', color: '#64748b' }}>{(file.size / 1024).toFixed(1)} KB · Ready to import</span>
                </div>
              ) : (
                <div>
        ...
        <div className="modal-footer">
          <button type="button" className="btn-white-secondary" onClick={onClose} disabled={busy}>Cancel</button>
          <button
            type="button"
            className="btn-orange-primary"
          ...
          >
            {busy ? 'Importing...' : <><Check size={16} /> Import Now</>}
          </button>
        </div>
      </div>
    </div>
