...
// // NOTE: using the new generic modal that supports Razorpay & Stripe


// import { toast } from 'react-toastify';
// import { smsIntegrationAPI } from '@/lib/smsIntegrationAPI';
// import { razorpayIntegrationAPI } from '@/lib/razorpayIntegrationAPI';
// // Placeholder import for stripe API wrapper — implement similarly to razorpayIntegrationAPI

// import PaymentProviderConfigurationModal from './SettingsModals/RazorpayConfigurationModal';
...
//   // generic provider modal state (works for Razorpay / Stripe)
//   const [providerModalOpen, setProviderModalOpen] = useState(false);
//   const [providerIntegration, setProviderIntegration] = useState<Integration | null>(null);
...
//       // NOTE: You can optionally load Stripe config here similarly (if you have stripeIntegrationAPI.getIntegration)
//       let stripeData: any = null;
//       try {
//         if (stripeIntegrationAPI && typeof stripeIntegrationAPI.getIntegration === 'function') {
//           const stripeResp = await stripeIntegrationAPI.getIntegration();
//           const sresp = stripeResp?.data ?? stripeResp ?? null;
//           if (sresp?.success && sresp.data) stripeData = sresp.data;
//         }
//       } catch (err) {
//         // ignore if stripe API not implemented
//         console.info('Stripe integration fetch skipped or failed', err);
//       }
...
//           category: 'payment',
//           provider: 'Stripe',
//           is_enabled: stripeData ? Boolean(stripeData?.is_active === 1 || stripeData?.is_active === true) : false,
//           is_connected: stripeData ? !!(stripeData?.publishable_key) : false,
//           config: {
...
//           webhook_url: stripeData?.webhook_url ?? 'https://your-domain.com/webhooks/stripe',
//           instructions:
//             'Configure Stripe by providing publishable & secret keys. Use PaymentIntents/Checkout for payments and verify webhooks on the server.',
//         },
//         {
//           id: '6',
...
//       } else if (integration.provider === 'Stripe' || integration.name === 'Stripe') {
//         // assume stripeIntegrationAPI exposes toggleIntegration
//         response = await stripeIntegrationAPI.toggleIntegration?.(payload);
//       } else {
...
//     // Open the generic payment provider modal for payment category
//     if (integration.category === 'payment' || integration.provider === 'Razorpay' || integration.provider === 'Stripe') {
//       setProviderIntegration(integration);
...
//   // Generic provider (Razorpay / Stripe) save handler
//   const handleSaveProviderConfiguration = async (data: {
//     provider: string;
//     keyId: string;
...
//       console.log('🔁 Sending provider payload to backend:', payload);

//       let response = null;
//       if (data.provider === 'Razorpay') {
//         response = await razorpayIntegrationAPI.saveIntegration(payload);
//       } else if (data.provider === 'Stripe') {
//         // stripeIntegrationAPI should be implemented on your backend wrapper library
//         // If you don't have it yet, implement similar to razorpayIntegrationAPI
//         response = await stripeIntegrationAPI.saveIntegration?.(payload);
//       } else {
//         // fallback: POST to a generic integrations endpoint (if available)
...
//       console.log('✅ Backend response (provider):', response);

//       await fetchIntegrations();
//       toast.success(`${data.provider} configuration saved successfully ✅`);

//       setProviderModalOpen(false);
//       setProviderIntegration(null);
//     } catch (error: any) {
//       console.error('❌ Error saving provider configuration:', error?.response?.data || error);
...
//       {/* Generic Payment Provider Configuration Modal (Razorpay / Stripe) */}
//       <PaymentProviderConfigurationModal
...
//           api_key: providerIntegration.api_key ?? null,
//           config: providerIntegration.config ?? null,
//           is_enabled: providerIntegration.is_enabled ?? false,
//         } : null}
//       />
...
// IntegrationsPage.tsx (UPDATED: separate Razorpay & Stripe modals)
import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import {
  ArrowLeft,
...
const IntegrationsPage: React.FC = () => {
  ...
  const handleConfigure = (integrationId: string) => {
    ...
    // Razorpay specific modal
    if (integration.name === 'Razorpay' || integration.provider === 'Razorpay' || integration.id === '5') {
    ...
    if (integration.name === 'Stripe' || integration.provider === 'Stripe' || integration.id === '5b') {
  ...
  // Razorpay save handler (provider-specific)
  ...
  // Stripe save handler (provider-specific)
  const handleSaveStripeConfiguration = async (data: {
    ...
    try {
      ...
      console.log('🔁 Sending Stripe payload to backend:', payload);

      const response = await stripeIntegrationAPI.saveIntegration(payload as any);
      console.log('✅ Backend response (Stripe):', response);
      ...
      console.error('❌ Error saving Stripe configuration:', error?.response?.data || error);
      toast.error(error?.response?.data?.message || 'Failed to save Stripe configuration');
    }
  };
