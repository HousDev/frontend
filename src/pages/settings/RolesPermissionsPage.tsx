...
// import React, { useState, useEffect } from "react";
// import { Link } from "react-router-dom";
// import { ArrowLeft, Shield, Edit, Lock, Save, Users, Key, ChevronRight, Search, User, CheckCircle2, XCircle } from "lucide-react";
// import { useAuth } from "@/contexts/AuthContext";
// import LoadingSpinner from "@/components/ui/LoadingSpinner";
// import { masterDataAPI } from "@/lib/mastersAPI";
// import { toast } from "react-toastify";
// import { rbacAPI } from "@/lib/rbacAPI";
// import { usersAPI } from "@/lib/api";  // or wherever usersAPI is exported from
...
// /** Convert module_permissions JSON → flat array of "resource.action" keys */
// function flattenModulePermissions(mp: Record<string, Record<string, boolean>> | null | undefined): string[] {
//   if (!mp) return [];
//   const keys: string[] = [];
...
// /** Convert flat ["resource.action"] → module_permissions JSON */
...
//   const [roles, setRoles]             = useState<Role[]>([]);
//   const [permissions]                 = useState<Permission[]>(STATIC_PERMISSIONS);
//   const [loading, setLoading]         = useState(true);
...
//   const hasUserPermission = (permId: string) => {
//     const perm = permissions.find((p) => p.id === permId);
//     if (!perm) return false;
//     return editingUserPerms.includes(`${perm.resource}.${perm.action}`);
//   };
...
//     const permission = permissions.find((p) => p.id === permissionId);
//     if (!permission) return false;
//     return rolePermissions.includes(`${permission.resource}.${permission.action}`);
//   };
...
//                   activeTab === tab ? "bg-orange-100 text-[#e87722]" : "bg-gray-100 text-gray-500"
//                 }`}>
//                   {tab === "roles" ? roles.length : tab === "permissions" ? permissions.length : allUsers.length}
//                 </span>
//               </button>
//             ))}
//           </nav>
...
//                       <div
//                         key={u.id}
//                         className="group relative border border-gray-200 rounded-xl p-4 hover:border-[#e87722] hover:shadow-md transition-all duration-200 bg-white overflow-hidden"
//                       >
//                         {/* Top accent on hover */}
//                         <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-[#1a3a5c] to-[#e87722] opacity-0 group-hover:opacity-100 transition-opacity" />
...
//                             <div className="min-w-0">
//                               <div className="flex items-center gap-1.5 flex-wrap">
//                                 <h3 className="text-sm font-bold text-[#1a3a5c] truncate">
...
//                             {permsFlat.length > 4 && (
//                               <span className="inline-block px-2 py-0.5 text-[10px] font-medium bg-gray-100 text-gray-500 rounded-full">
//                                 +{permsFlat.length - 4} more
//                               </span>
//                             )}
//                             {permsFlat.length === 0 && (
//                               <span className="text-[10px] text-gray-300 italic">No custom permissions</span>
//                             )}
//                           </div>
//                         </div>
//                       </div>
...
//                   <div key={resource} className="border border-gray-200 rounded-xl overflow-hidden">
//                     <div className={`px-4 py-2.5 ${meta.bg} border-b border-gray-200`}>
//                       <h5 className={`text-xs font-bold capitalize ${meta.color}`}>{resource} Permissions</h5>
//                     </div>
//                     <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-0 divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
//                       {resourcePermissions.map((permission) => {
//                         const isChecked = hasPermission(permission.id, editingRole.permissions);
...
//                     </h3>
//                     <p className="text-xs text-[#e87722] font-medium truncate">
//                       {editingUser.role}{editingUser.department ? ` · ${editingUser.department}` : ""}
//                     </p>
//                   </div>
//                 </div>
//                 <div className="flex items-center gap-2 shrink-0 ml-3">
//                   <span className="hidden sm:inline text-xs text-gray-400 bg-gray-100 px-2 py-1 rounded-lg">
//                     {editingUserPerms.length} selected
//                   </span>
//                   <button
...
//               {Object.entries(groupPermissionsByResource()).map(([resource, resourcePermissions]) => {
//                 const meta = resourceMeta[resource] || { color: "text-gray-700", bg: "bg-gray-50 border-gray-200" };
//                 const checkedCount = resourcePermissions.filter((p) => hasUserPermission(p.id)).length;
//                 return (
//                   <div key={resource} className="border border-gray-200 rounded-xl overflow-hidden">
//                     <div className={`px-4 py-2.5 ${meta.bg} border-b border-gray-200 flex items-center justify-between`}>
//                       <h5 className={`text-xs font-bold capitalize ${meta.color}`}>{resource} Permissions</h5>
//                       <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${checkedCount > 0 ? 'bg-orange-100 text-[#e87722]' : 'bg-white/70 text-gray-400'}`}>
//                         {checkedCount}/{resourcePermissions.length}
//                       </span>
//                     </div>
//                     <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-0 divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
//                       {resourcePermissions.map((permission) => {
//                         const isChecked = hasUserPermission(permission.id);
//                         return (
//                           <label
//                             key={permission.id}
//                             className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors hover:bg-orange-50/50 ${isChecked ? 'bg-orange-50/30' : ''}`}
...
//               </button>
//               <span className="ml-auto text-xs text-gray-400 hidden sm:inline">
//                 {editingUserPerms.length} permissions selected
//               </span>
//             </div>
//           </div>
//         </div>
