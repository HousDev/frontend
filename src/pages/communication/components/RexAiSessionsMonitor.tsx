            ...
            <>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {selectedSession.lead_id && (
                          <Link
                            to={`/dashboard/leads`}
                            className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-100 text-blue-800 hover:bg-blue-200 transition-colors flex items-center gap-1"
                          >
                            CRM Lead #{selectedSession.lead_id} <ArrowUpRight size={10} />
                          </Link>
                        )}
                      </div>
                      <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500 mt-0.5">
                        {getClientContact(selectedSession).phone && (
                          <span className="flex items-center gap-1 text-slate-700 font-medium">
                            ...
                            <a href={`tel:${getClientContact(selectedSession).phone}`} className="hover:underline">
                              {getClientContact(selectedSession).phone}
                            </a>
                          </span>
                        )}
                        {getClientContact(selectedSession).email && (
                          <span className="flex items-center gap-1 text-slate-700">
                            ...
                            <a href={`mailto:${getClientContact(selectedSession).email}`} className="hover:underline">
                              {getClientContact(selectedSession).email}
                            </a>
                          </span>
                        )}
                        <span className="flex items-center gap-1 text-[11px] text-slate-400 font-medium">
                          <Clock size={11} />
                          Started: {selectedSession.created_at ? format(new Date(selectedSession.created_at), "dd MMM yyyy, hh:mm a") : "N/A"}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-2.5 py-1.5 text-xs font-medium bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg transition-colors flex items-center gap-1.5"
                    ...
                    {getClientContact(selectedSession).phone && (
                      <a
                        ...
                        className="px-3 py-1.5 text-xs font-medium bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-colors flex items-center gap-1.5 shadow-xs"
                      >
                        <Phone size={13} />
                        WhatsApp Client
                      </a>
                  ...
                  <div className="mt-3 p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-xs">
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-[11px] font-bold uppercase tracking-wider text-slate-700 flex items-center gap-1">
                        <Compass size={12} style={{ color: BRAND.orange }} /> AI Extracted Client Preferences
                      </span>
                      <span className="text-[10px] text-slate-500 font-medium">
                        Intent: <strong className="text-slate-800 uppercase">{selectedSession.current_intent || "Buyer"}</strong>
                      </span>
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px]">
                      ...
                      <div className="bg-white p-2 rounded-lg border border-slate-200 shadow-2xs">
                        <span className="text-slate-400 block text-[10px] font-medium">Property Category</span>
                        <span className="font-bold text-slate-800">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {(!selectedSession.message_history || selectedSession.message_history.length === 0) ? (
                  ...
                  selectedSession.message_history.map((msg, index) => {
