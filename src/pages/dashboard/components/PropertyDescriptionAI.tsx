...
type Props<T extends WithDescription> = {
  formData: T;
  setFormData: (updater: (prev: T) => T) => void;
  /** Optional custom API endpoint; if not given, uses propertiesAPI helper */
  endpoint?: string; // e.g. "/api/ai/generate-description"
  /** Optional overrides (agar future me change karna ho to) */
  defaults?: {
...
export default function PropertyDescriptionAI<T extends WithDescription>({
  ...
  const tone: "professional" | "friendly" | "luxury" =
      defaults?.tone ?? "professional";
  const lang: "English" | "Hindi" | "Hinglish" = defaults?.lang ?? "English";
  const words = Number.isFinite(defaults?.words as number)
    ? Number(defaults!.words)
    : 120;
    ...
    try {
      ...
      if (endpoint) {
        const res = await fetch(endpoint, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ formData: selected, tone, lang, words }),
        });
      ...
      if (!text) throw new Error("Server did not return a description.");
      setFormData((prev) => ({ ...prev, description: text } as T));
    } catch (e: any) {
      const msg =
        e?.response?.data?.error ||
  ...
  return (
    <div className="space-y-2 p-0">
      ...
      <div className="relative">
        ...
        {/* Single quick-generate button (no settings UI) */}
        <button
          type="button"
          onClick={generateWithAI}
          disabled={loading}
      ...
      <p className="text-[11px] text-gray-500">
              AI uses: Property Type, Subtype, Unit Type, Furnishing, City, Location, budget,
