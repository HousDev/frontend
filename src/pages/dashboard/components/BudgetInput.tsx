...
//   // naive formatting assuming input is digits only
//   if (!s) return "";
//   const num = BigInt(s);
...
// const BudgetInput: React.FC<BudgetInputProps> = ({ value, onChange, error = null, onFocus }) => {
//   const [inWords, setInWords] = useState<string>("");

//   const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
//     const raw = e.target.value;
//     // remove all non-digit characters (so commas, spaces etc. are allowed in input)
//     const cleaned = raw.replace(/\D+/g, "");
//     // update parent with cleaned digits string
//     onChange(cleaned);
//     const num = cleaned ? Number(cleaned) : NaN;
//     setInWords(!Number.isNaN(num) && num > 0 ? numberToWords(num) : "");
//   };

//   // display value with commas for user friendliness while keeping underlying value digits-only
...
//       {inWords && <p className="text-xs text-green-600 mt-2">{inWords} Rupees</p>}
//     </div>
//   );
// };
