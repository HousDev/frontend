...
/* ---------------- SafeDropdown wrapper ----------------
   Many Dropdown implementations in projects don't type additional props
   like `searchable`. To avoid TS errors in this file while letting you
   use those props, we forward everything through an `any`-typed wrapper.
------------------------------------------------------*/
...
const PropertyFormModal: React.FC<PropertyFormModalProps> = ({
  ...
  const getLabelFromValue = (options: MasterOption[], value: string) => options.find(o => o.value === value)?.label || '';
