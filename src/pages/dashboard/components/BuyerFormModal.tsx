...
type Lead = {
  ...
  // added for notification update flow (reuse if present)
  assignment_notification_id?: number | null;
...
interface BuyerFormModalProps {
  ...
  followups?: Followup[];
  onClose: () => void;
  onTransferSuccess?: (transferredBuyer: any) => void;
}

/** ---------- Helpers to normalize dashboard paths ---------- */
...
function buildBuyerPath(basePrefix: string, buyerId?: string | number) {
  const id = buyerId != null ? String(buyerId) : "";
  const base = (basePrefix || "/dashboard/buyers").replace(/\/+$/g, "");
  if (!id) return base;
  return `${base}/${encodeURIComponent(id)}`;
}

/** ---------- Unified notifier (update-only) ---------- */
async function notifyAssignOrUpdate({
  lead,
  userId,
  message,
  type,
  ...
  // try to reuse any prior notification id if present on lead
  const maybeId =
    Number(
      (lead?.assignment_notification_id ?? lead?.notification_id) ?? 0
    ) || null;
...
const BuyerFormModal: React.FC<BuyerFormModalProps> = ({
  ...
  useEffect(() => {
    ...
    if ((lead as any)["budget_min"] != null || (lead as any)["budget_max"] != null) {
      budgetMin = Number((lead as any).budget_min) || 0;
      budgetMax = Number((lead as any).budget_max) || 0;
    } else if ((lead as any).budget_range) {
      const parts = String((lead as any).budget_range).split("-").map((p) => parseFloat(p) || 0);
    ...
    for (const k of possibleKeys) {
      ...
      if (Array.isArray(arr) && arr.length) {
        return arr.map((it: any) =>
          typeof it === "string" ? it : it.name ?? it.label ?? it.title ?? it.value ?? it.id
        );
      }
    }
    return [];
  ...
  const getAssignedExecName = () => {
    ...
    if (
      ...
      const salesExecByName = salesUsers.find((u) => {
        ...
        return String(userName || "").toLowerCase() === leadExecName.toLowerCase();
      });
  ...
  // Assign executive (UPDATE notification only)
  const handleExecAssign = async (execId: string) => {
    try {
      const previousExec = formData.assigned_executive;
        ...
        if (execId !== previousExec) {
          try {
            await notifyAssignOrUpdate({
              ...
              message: `Buyer lead assigned to ${execName}`,
              type: "buyer_assign",
              // force path style: /dashboard/leads/:id while still a lead
              link: buildLeadPath("/dashboard/leads", lead?.id),
            });
            console.log("✅ Assignment notification updated/created via update:", execName);
          } catch (notifErr) {
            console.error("Failed to update assignment notification:", notifErr);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // after transfer, send update-notification to assigned exec (if any)
      if (formData.assigned_executive && String(formData.assigned_executive).trim() !== "") {
        try {
          const execId = String(formData.assigned_executive);
          const exec =
          ...
          const buyerId = response?.data?.id || response?.id || lead.id;
          await notifyAssignOrUpdate({
            lead,
            userId: Number(execId),
            message: `New buyer transferred and assigned to ${execName}`,
            type: "buyer_transfer",
            // now point to buyers page with the resultant buyer id
            link: buildBuyerPath("/dashboard/buyers", buyerId),
          });

          console.log("✅ Transfer notification updated via update:", execName);
        } catch (notifErr) {
          console.error("Failed to update transfer notification:", notifErr);
        }
      }

      // notify lead creator (if different)
      if (lead.created_by && String(lead.created_by) !== String(user?.id)) {
        try {
          const buyerId = response?.data?.id || response?.id || lead.id;
          await notifyAssignOrUpdate({
          ...
          console.log("✅ Creator notification updated via update");
        } catch (notifErr) {
          console.error("Failed to update creator notification:", notifErr);
        }
      }
                            ...
                            return (
                              <>
                                ...
                                {availableSalesExecs.map((exec: any) => (
                                  ...
                                  >
                                    {exec.name}
                                    {exec.selfOnly && <span className="text-[10px] text-gray-400 ml-1">(Self)</span>}
                                  </button>
                                ))}
                              </>
                            );
                    ...
                    {showUnitTypeDropdown && unitTypeButtonRef.current && (
                      <div
                        ...
                        style={{
                          ...
                          zIndex: 9999,
                          top:
                            unitTypeButtonRef.current.getBoundingClientRect().bottom +
                            window.scrollY +
                            4,
                          left:
                            unitTypeButtonRef.current.getBoundingClientRect().left +
                            window.scrollX,
                          width:
                      ...
                      >
                        {(unitTypeOptions.length > 0
                          ...
                          <label
                            key={unitType}
                            className="flex items-center p-2 hover:bg-gray-50 cursor-pointer text-xs"
                          >
                            <input
                              type="checkbox"
                              checked={formData.preferred_unit_type?.includes(unitType)}
                              onChange={() =>
                                handleCheckboxChange("preferred_unit_type", unitType)
                              }
                    ...
                    {showLocationDropdown && locationButtonRef.current && (
                      <div
                        ...
                        style={{
                          ...
                          zIndex: 9999,
                          top:
                            locationButtonRef.current.getBoundingClientRect().bottom +
                            window.scrollY +
                            4,
                          left:
                            locationButtonRef.current.getBoundingClientRect().left +
                            window.scrollX,
                          width:
                        ...
                        {(locationOptions.length > 0
                          ...
                          >
                            <input
                              ...
                              onChange={() =>
                                handleCheckboxChange("preferred_location", location)
                              }
                              className="mr-2 h-3 w-3"
                            />
                            <span className="text-xs">{location}</span>
                  ...
                  <BudgetRangeSelector
                    initialMin={
                      ...
                      (formData?.budget_range
                        ? parseFloat(String(formData.budget_range).split("-")[0]) || 0.32
                        : 0.32)
                    }
                    initialMax={
                      Number(formData?.budget_max) ||
                      (formData?.budget_range
                        ? parseFloat(String(formData.budget_range).split("-")[1]) || 4.95
                        : 4.95)
                    }
                    max={50}
                    onChange={({ min, max: maxV, readable }) => {
