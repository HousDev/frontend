...
//         return parseInt(raw.replace(/\D/g, "")) * 10000000; // Convert Cr → ₹
//       }
//       if (raw.toLowerCase().includes("l")) {
//         return parseInt(raw.replace(/\D/g, "")) * 100000; // Convert L → ₹
//       }
...
//   // 1. New Listing - within 7 days of creation
...
//   // 2. Hot Deal - high interest (hot leads > 3) or many visits
...
//   // 3. Verified Property - has ownership document
...
//   // 4. Urgent Sale - based on selling rights or stage
...
//   // 5. Exclusive - based on selling rights
...
//   // 6. Direct Owner - based on lead source
...
//   // 7. Featured - if property is public and has good views
...
//   // 8. Recently Updated - within 2 days of update (but not new)
...
//   // 9. Best Price - budget is below market average
//   const budget = Number(property.budget) || 0;
//   if (budget > 0 && budget < 5_000_000) { // Less than 50L for residential
//     tags.push({
//       label: 'Best Price',
//       emoji: '🏷️',
//       priority: 2
...
//   // 10. Premium Property - high budget
//   if (budget > 20_000_000) { // More than 2Cr
//     tags.push({
//       label: 'Premium',
//       emoji: '💎',
//       priority: 1
...
//   // Sort by priority (higher priority first) and return max 3 tags
//   return tags
//     .sort((a, b) => b.priority - a.priority)
//     .slice(0, 3);
// };

// // --- Professional tone map for chips ---
...
//   "Premium": {
//     bg: "bg-gradient-to-r from-amber-300 via-yellow-500 to-amber-600",
...
// /** Image component that logs success/failure reasons and never shows fallback image */
...
//           hints: [
//             `Open ${resolved} in a new tab.`,
//             'Backend should serve app.use("/uploads", express.static(...)).',
//             'If DB stored absolute /uploads path already, return as-is or store relative "properties/<file>".',
//           ],
//         });
//       }}
//     />
...
// // Accept: string[] | comma-separated string | object of truthy flags -> string[]
...
//     r?.furnishing_items ??
//     r?.furnished_items ??
//     r?.furnishing_details ??
//     r?.furnishing_list ??
//     r?.furnishingItem ??
//     r?.furnishing_item ??
//     (typeof r?.furnishing === 'string' && r.furnishing.includes(',') ? r.furnishing : undefined);
...
//     // FIXED: Add ownership document fields
...
//     // FIXED: Proper ownership document mapping
...
//     // Existing photos; if you have real ids from backend, map them here
...
//   // NEW: Bulk operation loading states
...
//     console.log(raw, "raw API response")
...
//         (filters.location === 'all' || p.location === filters.location || p.city === filters.location) && // Location check
//         // ✅ Stage fix: convert "Deal Closure" → "deal_closure"
//         (filters.stage === 'all' || p.stage === filters.stage.toLowerCase().replace(/\s+/g, "_")) &&
//         // ✅ Price Range INR values (Lakhs / Crores format)
//         (filters.priceRange === 'all' || checkPriceRange(Number(p.budget), filters.priceRange)) &&
//         (filters.tags === 'all' || getPropertyTags(p).some(tag => tag.label === filters.tags)); // ADDED TAG FILTERING
...
//   const handleEditProperty = (property: UIProperty) => {
...
//   // NEW: Integrated Bulk Operations with API
...
//         updatedBy: 'User' // You might want to get this from user context
//       });
...
//                 </p>
//               </div>
//               <Globe size={18} className="text-orange-200" />
//             </div>
//           </div>
//         </div>
...
//           {/* Left side - Search + Filters + View toggle */}
...
//                   {masters?.["property status"]?.map(opt => (
//                     <option key={opt.value} value={opt.value}>
//                       {opt.label}
//                     </option>
//                   ))}
...
//                   {masters?.["property stages"]?.map(opt => (
//                     <option key={opt.value} value={opt.value}>
//                       {opt.label}
//                     </option>
//                   ))}
...
//                   {getUniquePropertyTags(properties).map(tag => (
...
//                     })
//                   }
//                   className="px-2.5 py-1 bg-gray-600 text-white rounded text-xs hover:bg-gray-700 transition-colors"
//                 >
//                   Reset
//                 </button>
//               </div>
...
//                         <span className="text-xs text-gray-500">{dash(property.propertyId)}</span>
//                       </div>
...
//                           <Building size={12} />
//                           <span>{dash(property.unitType)} • {dash(property.carpetArea)} sq ft</span>
//                         </div>
//                         <div className="flex items-center space-x-2 text-sm text-gray-600">
//                           <MapPin size={12} />
//                           <span>{dash(property.location)}, {dash(property.city)}</span>
//                         </div>
//                         <div className="flex items-center space-x-2 text-sm text-gray-600">
//                           <User size={12} />
...
//                           <div className="text-sm font-medium">{dash(p.location)}, {dash(p.city)}</div>
      ...
      <div className="bg-white border-b border-gray-200 px-4 lg:px-6 py-3">
        ...
        {showFilters && (
          <div className="bg-gray-50 rounded-lg p-3 mt-3">
            <div className="grid grid-cols-2 md:grid-cols-7 gap-2">
              <div>
                <label className="block text-[11px] font-medium text-gray-700 mb-1">Property Type</label>
                <select
                  value={filters.type}
                  onChange={(e) => setFilters(prev => ({ ...prev, type: e.target.value }))}
                  className="w-full px-2 py-1 border border-gray-300 rounded text-xs"
              ...
              <div>
                <label className="block text-[11px] font-medium text-gray-700 mb-1">Price Range</label>
                <select
                  value={filters.priceRange}
                  onChange={(e) => setFilters(prev => ({ ...prev, priceRange: e.target.value }))}
                  className="w-full px-2 py-1 border border-gray-300 rounded text-xs"
              ...
              <div>
                <label className="block text-[11px] font-medium text-gray-700 mb-1">Stage</label>
                <select
                  value={filters.stage}
                  onChange={(e) => setFilters(prev => ({ ...prev, stage: e.target.value }))}
                  className="w-full px-2 py-1 border border-gray-300 rounded text-xs"
