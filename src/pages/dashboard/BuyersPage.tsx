...
//     // accept multiple shapes + coerce to string
//     const reqPropertyType =
//       rawReq.propertyType ??
//       rawReq.property_type ??
//       b.propertyType ??
//       b.property_type ??
//       null;
...
//       requirements: {
//         propertyType: toStr(reqPropertyType),             // <- robust
...
//         setBuyers([]); // no static fallback; keep empty list
//       }
//     };
//     fetchBuyers();
//   }, []);
// console.log('stages:', buyers.map(b => b.stage));
// console.log('is_active:', buyers.map(b => b.is_active));

//   // -------- Derived lists & helpers (unchanged UI; safe fallbacks) ----------
// const tabs = [
//   { id: 'all', label: 'All', count: buyers.length, color: 'blue' },

//   { id: 'hot_leads', label: 'Hot Leads',
//     count: buyers.filter(b => (b.priority ?? '').toLowerCase() === 'high').length,
//     color: 'red'
//   },

//   // 👇 Active tab ab is_active check karega, status nahi
//   { id: 'active', label: 'Active',
//     count: buyers.filter(b => b.is_active === true).length,
//     color: 'green'
//   },

//   { id: 'property_hunting', label: 'Property Hunting',
//     count: buyers.filter(b => (b.stage ?? '').toLowerCase().replace(/\s+/g, '_') === 'property_hunting').length,
//     color: 'purple'
//   },

//   { id: 'loan_processing', label: 'Loan Processing',
//     count: buyers.filter(b => (b.stage ?? '').toLowerCase().replace(/\s+/g, '_') === 'loan_processing').length,
//     color: 'orange'
//   },

//   { id: 'ready_to_buy', label: 'Ready to Buy',
//     count: buyers.filter(b => (b.stage ?? '').toLowerCase().replace(/\s+/g, '_') === 'property_finalization').length,
//     color: 'indigo'
//   }
// ];
...
//       (activeTab === 'hot_leads' && (buyer.priority ?? '') === 'high') ||
//       (activeTab === 'active' && (buyer.status ?? '') === 'active') ||
//       (activeTab === 'property_hunting' && (buyer.stage ?? '') === 'property_hunting') ||
//       (activeTab === 'loan_processing' && (buyer.stage ?? '') === 'loan_processing') ||
//       (activeTab === 'ready_to_buy' && (buyer.stage ?? '') === 'property_finalization');

//     const matchesFilters =
//       (filters.source === 'all' || (buyer.source ?? '') === filters.source) &&
//       (filters.stage === 'all' || (buyer.stage ?? '') === filters.stage) &&
//       (filters.priority === 'all' || (buyer.priority ?? '') === filters.priority) &&
//       (filters.assigned === 'all' || (buyer.assigned ?? '') === filters.assigned) &&
//       (filters.status === 'all' || (buyer.status ?? '') === filters.status) &&
//       (filters.propertyType === 'all' || (buyer.requirements?.propertyType ?? '') === filters.propertyType);
...
//   // 🔧 UPDATED: call delete API before removing locally
...
//   // 🔧 UPDATED: call update API when editing; keep local add for new buyers
...
//         );
//       } else {
//         // no create API requested—keep your existing local add
...
//     // No static defaults — just normalize what we get
...
//   // ------- Badges & small render helpers (unchanged UI; safe data) -------
...
//     // Accept either: "property_hunting", "Property Hunting" or a buyer object
//     const raw =
...
//     // Normalization map for common backend label formats -> canonical keys
...
//       'completed': 'completed',
//       // also allow some variants
//       'property-hunting': 'property_hunting',
//       'property_hunting': 'property_hunting',
//       'propertyfinalization': 'property_finalization',
//     };

//     // Normalize: lowercase, trim, then map spaces/dashes to underscores
//     const lower = rawStr.toLowerCase();
//     let key = labelToKey[lower] ?? lower.replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '');

//     // If key still not in config, try fuzzy match (contains or contained)
//     if (!stageConfig[key]) {
//       const fuzzy = Object.keys(stageConfig).find(k => k.includes(key) || key.includes(k));
//       if (fuzzy) key = fuzzy;
//     }

//     // Final fallback
//     if (!stageConfig[key]) key = 'initial_contact';
...
//     const key = (["high", "medium", "low"].includes(raw) ? raw : alias[raw]) ?? "";
...
//     // Always have a fallback config to avoid undefined.bg
//     const fallbackLabel = raw ? raw.replace(/\b\w/g, c => c.toUpperCase()) : "—";
//     const config = priorityConfig[key] ?? { bg: "bg-gray-100", text: "text-gray-700", label: fallbackLabel, icon: "" };
...
//             onBack={() => setCurrentBuyerAccount(null)}
//           />
//         ) : currentBuyerView ? (
//           <BuyerViewPage
...
//           />
//         ) : (
//           <div>{/* list placeholder, unchanged */}</div>
//         )}
...
//           {stages.filter(s => s !== 'all').slice(0, 4).map(stage => (
//             <button
//               key={stage}
...
//                   onChange={(e) => setFilters(prev => ({ ...prev, ignoreDate: e.target.checked }))}
...
//                   className="flex items-center space-x-1 px-3 py-1 text-gray-600 hover:text-gray-800 text-xs"
//                 >
//                   <X size={12} />
//                   <span>Reset</span>
//                 </button>
...
//                           {safeStr(buyer.salutation)} {safeStr(buyer.name)}
//                         </div>
