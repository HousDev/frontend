...
const normalizeBuyer = (b: any) => {
  ...
  useEffect(() => {
    const fetchBuyers = async () => {
      try {
        ...
        const apiBuyers = await buyerAPI.getAll();
        const normalized = Array.isArray(apiBuyers) ? apiBuyers.map(normalizeBuyer) : [];
        // keep consistent newest-first by created_at (fallback: lastActivity)
  ...
  // keep the filtered list also newest-first (consistent with initial load and after create)
...
const handleSaveBuyer = async (buyerData: any) => {
  ...
  const handleImportBuyers = (buyersData: any[]) => {
    const newBuyers = buyersData.map(normalizeBuyer).map(b => ({
      ...b,
      created_at: b.created_at || new Date().toISOString(),
    }));
    // Import to TOP and keep sort consistent
    setBuyers(prev => {
      const next = [...newBuyers, ...prev];
    ...
    toast.success(`Imported ${newBuyers.length} buyers.`);
  };
  ...
  if (currentBuyerView) {
    return (
      <div className="buyers-page">
        ...
        />
        {/* keep modals mount here if needed */}
        {showBuyerForm && (
          <BuyerFormModal
            isOpen={showBuyerForm}
            buyer={editingBuyer}
