...
//   // 🔧 Pagination: default 5 and options 5/10/15/20
...
//       id: raw.id ?? raw._id ?? raw.slug ?? `LOCAL_${Date.now()}`,
...
//       featuredImage: raw.featuredImage ?? raw.featured_image ?? raw.image ?? raw.imageUrl ?? '',
...
//       seoTitle: raw.seoTitle ?? raw.seo_title ?? raw.metaTitle ?? '',
//       seoDescription: raw.seoDescription ?? raw.seo_description ?? raw.metaDescription ?? '',
//       readTime: typeof raw.readTime === 'number' ? raw.readTime : Math.ceil(((raw.content || '').length || 0) / 200),
//       // @ts-ignore
//       slug: raw.slug
//     };
...
//   const closePreviewModal = () => { setShowPreviewModal(false); setPreviewPost(null); };
...
//     if (!postId) { setCommentsForPost([]); return; }
...
//         if (p && (p as any).commentsList && Array.isArray((p as any).commentsList)) setCommentsForPost((p as any).commentsList);
...
//         status: createdObjRaw?.status ?? (shouldAutoPublish ? 'published' : 'draft'),
//         featured: createdObjRaw?.featured ?? payload.featured ?? false,
//         featuredImage: createdObjRaw?.featuredImage ?? createdObjRaw?.featured_image ?? createdObjRaw?.image ?? createdObjRaw?.imageUrl ?? payload.featuredImage ?? '',
//         publishedAt: createdObjRaw?.publishedAt ?? createdObjRaw?.published_at ?? (shouldAutoPublish ? new Date().toISOString() : ''),
...
//         await (blogsAPI as any).updatePost(postId, { status: 'published', publishedAt: new Date().toISOString() });
//       }
...
//       {/* cards... (unchanged) */}
//       {/* ...same as previous message (kept for brevity) */}
//       {/* I kept the entire original renderDashboard content from last reply */}
//       {/* START copy of previous content */}
...
//               className={`px-4 py-2 rounded-full text-sm font-medium transition ${postStateTab === 'published' ? 'bg-white shadow border text-gray-900' : 'text-gray-600 hover:text-gray-900'}`}
//             >
//               Published
//             </button>
//           </div>
...
//           {/* 🔧 Page size selector with 5/10/15/20 */}
...
//                             </button>
//                             <button
//                               onClick={() => rewriteThenPublish(post.id!)}
//                               className="px-2 py-1 text-xs rounded-md bg-gradient-to-r from-purple-600 to-blue-600 text-white hover:opacity-90"
//                               title="Rewrite with AI then auto-publish"
//                             >
//                               Rewrite → Publish
//                             </button>
//                           </>
...
//           </div>
//           <div className="flex items-center gap-2">
//             <button onClick={() => setPage(1)} disabled={currentPage === 1} className={`px-3 py-2 border rounded-md ${currentPage === 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-50'}`}>First</button>
//             <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className={`px-3 py-2 border rounded-md ${currentPage === 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-50'}`}>Prev</button>
//             <span className="px-3 py-2 text-sm">Page <span className="font-semibold">{currentPage}</span> / {totalPages}</span>
//             <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className={`px-3 py-2 border rounded-md ${currentPage === totalPages ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-50'}`}>Next</button>
//             <button onClick={() => setPage(totalPages)} disabled={currentPage === totalPages} className={`px-3 py-2 border rounded-md ${currentPage === totalPages ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-50'}`}>Last</button>
//             <button
//               onClick={handleRefresh}
//               disabled={loadingPosts}
...
//   // --- renderCommentsTab, renderAITools, renderSEOTools remain same as previous message ---
//   // (Keeping them identical to last version to avoid noise)
//   // -- START: Same as previous message (no pagination changes inside) --
...
//                   <button
//                     onClick={() => { setShowPostEditor(true); setSelectedPost(activeCommentsPost); }}
...
//             <div className="mb-4">
//               <CommentComposer onPost={(text, author, email) => postNewCommentOnActivePost(text, author, email)} posting={false} />
//             </div>
...
//                                 </div>
//                                 <div className="min-w-0">
//                                   <div className="text-sm font-medium text-gray-900 truncate">{reply.author}</div>
...
//               <div className="p-2 bg-green-100 rounded-lg"><Save className="text-green-600" size={20} /></div>
...
//               <div className="flex justify-between text-sm"><span>Accuracy Rate</span><span className="font-bold text-green-600">99.7%</span></div>
//               <div className="flex justify-between text-sm"><span>Avg Originality</span><span className="font-bold text-green-600">97.2%</span></div>
//             </div>
...
//             <div className="space-y-2">
//               <div className="flex justify-between text-sm"><span>Avg SEO Score</span><span className="font-bold text-blue-600">89.5%</span></div>
//               <div className="flex justify-between text-sm"><span>Posts Optimized</span><span className="font-bold text-blue-600">{postsArray.length}</span></div>
//             </div>
...
//             <div className="flex items-center space-x-3 mb-4">
//               <div className="p-2 bg-purple-100 rounded-lg"><Wand2 className="text-purple-600" size={20} /></div>
...
//             <div className="space-y-2">
//               <div className="flex justify-between text-sm"><span>Enhancement Rate</span><span className="font-bold text-purple-600">92.8%</span></div>
//               <div className="flex justify-between text-sm"><span>Improved Posts</span><span className="font-bold text-purple-600">{Math.floor(postsArray.length * 0.8)}</span></div>
//             </div>
...
//                 <div className="space-y-2">
//                   <div className="flex justify-between text-sm"><span>"real estate investment"</span><span className="font-bold text-blue-600">1,200 searches</span></div>
//                   <div className="flex justify-between text-sm"><span>"mumbai property"</span><span className="font-bold text-blue-600">890 searches</span></div>
//                   <div className="flex justify-between text-sm"><span>"home buying guide"</span><span className="font-bold text-blue-600">650 searches</span></div>
//                 </div>
//               </div>
//             </div>
//           </div>
...
//               <AIBlogWriter
//                 isOpen={true}
//                 onClose={() => { setShowAIWriter(false); setSelectedPost(null); setAutoPublishAfterAI(null); }}
