...
#RealEstate #Property #Mumbai #Investment #PropertyTips #HomeSearch #${post.category.replace(/\s/g, '')}`
  ...
  const StatCard = ({ icon: Icon, label, value, color }: any) => (
    <div className="bg-white rounded-lg p-2.5 transition-all hover:shadow-md" style={{ border: `1px solid ${BD}` }}>
      <div className="flex items-center justify-between">
        <div>
          <p className="text-[9px] uppercase tracking-wider" style={{ color: MU }}>{label}</p>
  ...
  const renderPostsTab = () => (
    <div className="space-y-3">
      <div className="bg-white rounded-xl shadow-sm p-3" style={{ border: `1px solid ${BD}` }}>
        <h3 className="text-sm font-bold mb-3" style={{ color: N }}>Published Blog Posts</h3>
        
        <div className="space-y-3">
          {posts.map((post) => (
            <div key={post.id} className="rounded-lg p-3 transition-all hover:shadow-sm" style={{ border: `1px solid ${BD}` }}>
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-bold mb-1 truncate" style={{ color: N }}>{post.title}</h4>
                  <p className="text-[10px] mb-2 line-clamp-2" style={{ color: MU }}>{post.excerpt}</p>
                  
                  <div className="flex items-center gap-2 mb-2">
                    <span className="px-1.5 py-0.5 rounded-full text-[9px] font-medium" style={{ background: `${O}10`, color: O }}>
                      {post.category}
                    </span>
                  ...
                  <div className="flex items-center justify-between">
                      ...
                      {platforms.filter(p => p.connected).map((platform) => {
                        ...
                        return (
                          <button
                            ...
                            style={!existingPost ? { border: `1px solid ${BD}` } : {}}
                            title={existingPost ? 'Already posted' : `Post to ${platform.name}`}
                          >
                            <Icon size={12} />
                            {existingPost && <CheckCircle size={8} />}
                          </button>
                        );
                      })}
                    </div>
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg text-white text-[10px] font-medium transition-all hover:opacity-90"
                      style={{ background: N }}
                    >
                      Schedule
                    </button>
        ...
        {posts.length === 0 && (
          <div className="text-center py-8">
            <Share2 size={32} style={{ color: MU }} className="mx-auto mb-2" />
            <h3 className="text-sm font-semibold mb-1" style={{ color: N }}>No Published Posts</h3>
            <p className="text-[10px]" style={{ color: MU }}>Publish blog posts to share on social media</p>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="space-y-3">
      <div className="bg-white rounded-xl shadow-sm p-3" style={{ border: `1px solid ${BD}` }}>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-bold" style={{ color: N }}>Scheduled Posts</h3>
          <button
            onClick={() => setShowScheduleModal(true)}
            className="px-2.5 py-1 rounded-lg text-white text-[10px] font-medium flex items-center gap-1 transition-all hover:opacity-90"
            style={{ background: N }}
          >
            <Plus size={10} style={{ color: O }} />
            <span>Schedule</span>
          </button>
        </div>
        ...
        <div className="space-y-2">
          {socialPosts
            ...
            .map((socialPost) => {
              ...
              return (
                <div key={socialPost.id} className="rounded-lg p-2.5" style={{ border: `1px solid ${BD}` }}>
                  <div className="flex gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-1">
                        <h4 className="text-[11px] font-semibold truncate" style={{ color: N }}>{blogPost?.title}</h4>
                        <span className="px-1.5 py-0.5 rounded-full text-[8px] font-medium" style={{ background: '#fef3c7', color: '#d97706' }}>
                          Scheduled
                        </span>
                      </div>
                      <p className="text-[9px] mb-1.5 line-clamp-2" style={{ color: MU }}>{socialPost.content}</p>
                      <div className="flex items-center gap-2 text-[9px]" style={{ color: MU }}>
                        <span className="flex items-center gap-0.5"><Calendar size={8} />{new Date(socialPost.scheduledDate).toLocaleDateString()}</span>
                        <span className="flex items-center gap-0.5"><Clock size={8} />{new Date(socialPost.scheduledDate).toLocaleTimeString()}</span>
                        <span className="capitalize flex items-center gap-0.5"><Globe size={8} />{socialPost.platform}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-1">
                      <button className="p-1 rounded transition-colors hover:bg-gray-100"><Eye size={10} style={{ color: MU }} /></button>
                      <button className="p-1 rounded transition-colors hover:bg-red-50"><X size={10} style={{ color: '#ef4444' }} /></button>
                    </div>
                  </div>
                </div>
              );
        ...
        {socialPosts.filter(sp => sp.status === 'scheduled').length === 0 && (
          <div className="text-center py-6">
            <Calendar size={28} style={{ color: MU }} className="mx-auto mb-2" />
            <h3 className="text-sm font-semibold mb-1" style={{ color: N }}>No Scheduled Posts</h3>
            <p className="text-[10px] mb-2" style={{ color: MU }}>Schedule blog posts for social media</p>
            <button
              onClick={() => setShowScheduleModal(true)}
              className="px-2.5 py-1 rounded-lg text-white text-[10px] font-medium transition-all hover:opacity-90"
              style={{ background: N }}
  ...
  const renderAnalyticsTab = () => (
    <div className="space-y-3">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        <StatCard icon={Share2} label="Total Posts" value={socialPosts.length} color="#3b82f6" />
        <StatCard icon={TrendingUp} label="Engagement" value={socialPosts.reduce((acc, sp) => acc + sp.engagement.likes + sp.engagement.shares + sp.engagement.comments, 0)} color="#10b981" />
        <StatCard icon={Users} label="Total Clicks" value={socialPosts.reduce((acc, sp) => acc + sp.engagement.clicks, 0)} color="#8b5cf6" />
        <StatCard icon={Target} label="Avg. Engagement" value="12.5%" color={O} />
      </div>

      <div className="bg-white rounded-xl shadow-sm p-3" style={{ border: `1px solid ${BD}` }}>
        <h3 className="text-sm font-bold mb-3" style={{ color: N }}>Performance by Platform</h3>
        ...
        <div className="space-y-2">
          {platforms.filter(p => p.connected).map((platform) => {
            ...
            return (
              <div key={platform.name} className="flex items-center justify-between p-2.5 rounded-lg" style={{ background: `${N}05`, border: `1px solid ${BD}` }}>
                <div className="flex items-center gap-2">
                  <Icon size={14} style={{ color: platform.color }} />
                  <div>
                    <h4 className="text-[11px] font-semibold" style={{ color: N }}>{platform.name}</h4>
                    <p className="text-[9px]" style={{ color: MU }}>{platformPosts.length} posts</p>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-sm font-bold" style={{ color: N }}>{totalEngagement}</div>
                  <div className="text-[8px]" style={{ color: MU }}>Engagement</div>
                </div>
              </div>
            );
          })}
  ...
  const renderSettingsTab = () => (
    <div className="space-y-3">
      <div className="bg-white rounded-xl shadow-sm p-3" style={{ border: `1px solid ${BD}` }}>
        <h3 className="text-sm font-bold mb-3" style={{ color: N }}>Platform Connections</h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {platforms.map((platform) => {
            const Icon = platform.icon;
            return (
              <div key={platform.name} className="rounded-lg p-2.5" style={{ border: `1px solid ${BD}` }}>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <Icon size={14} style={{ color: platform.color }} />
                    <div>
                      <h4 className="text-[11px] font-semibold" style={{ color: N }}>{platform.name}</h4>
                      <p className="text-[8px]" style={{ color: MU }}>Limit: {platform.characterLimit}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1">
                    <div className={`w-1.5 h-1.5 rounded-full ${platform.connected ? 'bg-green-500' : 'bg-gray-300'}`} />
                    <span className={`text-[8px] font-medium ${platform.connected ? 'text-green-600' : 'text-gray-500'}`}>
                ...
                <div className="flex flex-wrap gap-1 mb-2">
                  {platform.features.slice(0, 2).map((feature, index) => (
                    <div key={index} className="flex items-center gap-0.5">
                      <CheckCircle size={7} style={{ color: '#10b981' }} />
                      <span className="text-[7px]" style={{ color: MU }}>{feature}</span>
                    </div>
                  ))}
                </div>

                <button
                  className={`w-full py-1.5 rounded-lg text-[9px] font-medium transition-colors ${
                    platform.connected
                      ? 'text-red-600 hover:bg-red-50'
                      : 'text-white hover:opacity-90'
                  }`}
                  style={!platform.connected ? { background: N } : { background: '#fee2e2' }}
                >
                  {platform.connected ? 'Disconnect' : 'Connect'}
                </button>
              </div>
  ...
  const renderContent = () => (
    <div className="space-y-3">
      ...
      <div className="bg-white rounded-xl shadow-sm p-3" style={{ border: `1px solid ${BD}` }}>
        <div className="flex items-center gap-2 mb-2">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: N }}>
            <Share2 size={14} style={{ color: O }} />
          </div>
          <div>
            <h2 className="text-sm font-bold" style={{ color: N }}>Social Media Management</h2>
            <p className="text-[9px]" style={{ color: MU }}>Schedule and share content across platforms</p>
          </div>
        </div>

        {/* Quick Stats */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          <div className="rounded-lg p-2" style={{ background: `${O}10` }}>
            <div className="flex items-center gap-1.5">
              <Share2 size={12} style={{ color: O }} />
              <div>
                <div className="text-sm font-bold" style={{ color: O }}>{socialPosts.filter(sp => sp.status === 'posted').length}</div>
                <div className="text-[8px]" style={{ color: MU }}>Posts Shared</div>
              </div>
            </div>
          </div>
          <div className="rounded-lg p-2" style={{ background: `${N}10` }}>
            <div className="flex items-center gap-1.5">
              <Calendar size={12} style={{ color: N }} />
              <div>
                <div className="text-sm font-bold" style={{ color: N }}>{socialPosts.filter(sp => sp.status === 'scheduled').length}</div>
                <div className="text-[8px]" style={{ color: MU }}>Scheduled</div>
              </div>
            </div>
          </div>
          <div className="rounded-lg p-2" style={{ background: '#f3e8ff' }}>
            <div className="flex items-center gap-1.5">
              ...
              <div>
                ...
                <div className="text-[8px]" style={{ color: MU }}>Total Clicks</div>
              </div>
            </div>
          </div>
      ...
      {/* Navigation Tabs */}
      <div className="bg-white rounded-xl shadow-sm" style={{ border: `1px solid ${BD}` }}>
        <div className="flex border-b" style={{ borderColor: BD }}>
          {tabs.map((tab) => {
            const Icon = tab.icon;
            return (
              <button
                ...
                style={activeTab === tab.id ? { borderBottomColor: O, color: N } : { color: MU }}
              >
                <Icon size={12} />
                <span>{tab.label}</span>
              </button>
      ...
      {showScheduleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2" style={{ background: 'rgba(15,43,61,0.6)', backdropFilter: 'blur(4px)' }}>
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full max-h-[85vh] overflow-y-auto" style={{ border: `1px solid ${BD}` }}>
            <div className="sticky top-0 px-3 py-2 border-b flex items-center justify-between" style={{ borderColor: BD, background: BG }}>
              <h3 className="text-sm font-bold" style={{ color: N }}>Schedule Post</h3>
              <button onClick={() => setShowScheduleModal(false)} className="p-1 rounded hover:bg-gray-100">
            ...
            <div className="p-3 space-y-3">
              <div>
                <label className="block text-[10px] font-medium mb-0.5" style={{ color: N }}>Select Blog Post</label>
                <select
                  value={selectedPost?.id || ''}
                  onChange={(e) => {
                    const post = posts.find(p => p.id === e.target.value);
              ...
              <div>
                <label className="block text-[10px] font-medium mb-0.5" style={{ color: N }}>Platform</label>
                  ...
                  {platforms.filter(p => p.connected).map((platform) => {
                    ...
                    return (
                      <button
                        ...
                        className={`p-1.5 border rounded-lg transition-colors text-center ${
                          scheduleData.platform === platform.name.toLowerCase()
                            ? 'border-orange-500 bg-orange-50'
                            : ''
                        }`}
                        style={scheduleData.platform === platform.name.toLowerCase() ? { borderColor: O } : { borderColor: BD }}
                      >
                        <Icon size={12} style={{ color: platform.color }} className="mx-auto" />
                        <div className="text-[8px] mt-0.5" style={{ color: MU }}>{platform.name}</div>
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[10px] font-medium mb-0.5" style={{ color: N }}>Date</label>
                  <input
                    type="date"
                    value={scheduleData.scheduledDate}
                    onChange={(e) => setScheduleData(prev => ({ ...prev, scheduledDate: e.target.value }))}
                ...
                <div>
                  <label className="block text-[10px] font-medium mb-0.5" style={{ color: N }}>Time</label>
                  <input
                    type="time"
                    value={scheduleData.scheduledTime}
                    onChange={(e) => setScheduleData(prev => ({ ...prev, scheduledTime: e.target.value }))}
              ...
              <div>
                <label className="block text-[10px] font-medium mb-0.5" style={{ color: N }}>Post Content</label>
                <div className="relative">
                  <textarea
                    value={scheduleData.content}
                    onChange={(e) => setScheduleData(prev => ({ ...prev, content: e.target.value }))}
                    rows={4}
                    className="w-full px-2 py-3.5 text-[10px] border rounded focus:outline-none focus:ring-1 pr-7 resize-none"
                    style={{ borderColor: BD }}
                    placeholder="Your social media post content..."
                  />
                  <button
                    type="button"
                    onClick={() => copyToClipboard(scheduleData.content)}
                    className="absolute top-1.5 right-1.5 p-1 rounded transition-colors hover:bg-gray-100"
                  >
                    <Copy size={10} style={{ color: MU }} />
                  </button>
                </div>
                <div className="text-[8px] mt-0.5" style={{ color: MU }}>
                  {scheduleData.content.length} / {platforms.find(p => p.name.toLowerCase() === scheduleData.platform)?.characterLimit || 2200} chars
                </div>
              </div>
              ...
              <label className="flex items-center gap-1.5 cursor-pointer">
                <input
                  ...
                  style={{ accentColor: O }}
                />
                <span className="text-[10px]" style={{ color: N }}>Include featured image</span>
              </label>
              ...
              <div className="flex gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => setShowScheduleModal(false)}
                  className="flex-1 px-2 py-1.5 text-[10px] border rounded transition-colors hover:bg-gray-50"
                ...
                <button
                  onClick={handleSchedulePost}
                  className="flex-1 px-2 py-1.5 text-[10px] rounded text-white flex items-center justify-center gap-1 transition-all hover:opacity-90"
                  style={{ background: N }}
                >
  ...
  return isOpen !== undefined ? (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2" style={{ background: 'rgba(15,43,61,0.6)', backdropFilter: 'blur(4px)' }}>
      <div className="bg-white rounded-xl shadow-2xl max-w-5xl w-full max-h-[88vh] overflow-y-auto" style={{ border: `1px solid ${BD}` }}>
        <div className="sticky top-0 z-10 flex items-center justify-between px-3 py-2 border-b" style={{ background: BG, borderColor: BD }}>
          <h2 className="text-sm font-bold" style={{ color: N }}>Social Media Management</h2>
