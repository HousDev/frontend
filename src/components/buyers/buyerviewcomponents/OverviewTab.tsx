...
// ESALE Theme Colors - Light Version
const theme = {
  primary: '#0f2b3d',
  primaryLight: '#e8edf0',
  primaryBg: '#f0f3f5',
...
const OverviewTab: React.FC<OverviewTabProps> = ({ buyer, onUpdateBuyer, setActiveTab }) => {
  ...
  const getStatusBadge = (status: string, size: string = 'text-xs') => {
    const statusConfig = {
      'active': { bg: 'bg-emerald-50', text: 'text-emerald-700', label: 'Active', icon: '🟢' },
      'inactive': { bg: 'bg-gray-50', text: 'text-gray-600', label: 'Inactive', icon: '⚫' },
      'blocked': { bg: 'bg-red-50', text: 'text-red-700', label: 'Blocked', icon: '🔴' }
    };
  ...
  return (
    <div className="space-y-6">
      {/* Key Metrics - Original UI with ESALE Light Colors */}
<div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 md:gap-4 lg:gap-6">
  {/* Lead Score Card */}
  <div className="rounded-lg p-2.5 sm:p-3 md:p-4 text-white" style={{ background: `linear-gradient(135deg, ${theme.primary} 0%, #1a4a6a 100%)` }}>
    <div className="flex items-center justify-between">
      <div>
        <p className="text-white/70 text-[9px] sm:text-[10px] md:text-xs uppercase tracking-wide">Lead Score</p>
        <p className="text-sm sm:text-base md:text-xl lg:text-2xl font-bold mt-0.5 sm:mt-1">{buyer.leadScore}</p>
      </div>
      <Star size={16} className="sm:size-[18px] md:size-[20px] lg:size-[24px] text-white/50" />
    </div>
  </div>

  {/* Properties Matched Card */}
  <div className="rounded-lg p-2.5 sm:p-3 md:p-4 text-white" style={{ background: `linear-gradient(135deg, ${theme.accent} 0%, #f39c12 100%)` }}>
    <div className="flex items-center justify-between">
      <div>
        <p className="text-white/70 text-[9px] sm:text-[10px] md:text-xs uppercase tracking-wide">Properties Matched</p>
        <p className="text-sm sm:text-base md:text-xl lg:text-2xl font-bold mt-0.5 sm:mt-1">{buyer.matchedProperties?.length || 0}</p>
      </div>
      <Building size={16} className="sm:size-[18px] md:size-[20px] lg:size-[24px] text-white/50" />
    </div>
  </div>

  {/* Total Visits Card */}
  <div className="rounded-lg p-2.5 sm:p-3 md:p-4 text-white" style={{ background: `linear-gradient(135deg, #1e6f5c 0%, #289672 100%)` }}>
    <div className="flex items-center justify-between">
      <div>
        <p className="text-white/70 text-[9px] sm:text-[10px] md:text-xs uppercase tracking-wide">Total Visits</p>
        <p className="text-sm sm:text-base md:text-xl lg:text-2xl font-bold mt-0.5 sm:mt-1">{buyer.visits || 0}</p>
      </div>
      <Eye size={16} className="sm:size-[18px] md:size-[20px] lg:size-[24px] text-white/50" />
    </div>
  </div>

  {/* Response Rate Card */}
  <div className="rounded-lg p-2.5 sm:p-3 md:p-4 text-white" style={{ background: `linear-gradient(135deg, #6c3483 0%, #8e44ad 100%)` }}>
    <div className="flex items-center justify-between">
      <div>
        <p className="text-white/70 text-[9px] sm:text-[10px] md:text-xs uppercase tracking-wide">Response Rate</p>
        <p className="text-sm sm:text-base md:text-xl lg:text-2xl font-bold mt-0.5 sm:mt-1">{buyer.responseRate || 80}%</p>
      </div>
      <TrendingUp size={16} className="sm:size-[18px] md:size-[20px] lg:size-[24px] text-white/50" />
    </div>
  </div>
</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Contact Information */}
        <div className="bg-white rounded-xl shadow-sm border p-4" style={{ borderColor: theme.border }}>
          <h3 className="text-sm font-semibold mb-3 flex items-center" style={{ color: theme.primary }}>
            <User className="mr-2" size={16} style={{ color: theme.accent }} />
            Contact Information
          </h3>
          <div className="space-y-2 text-xs grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2">
            {/* Phone */}
        ...
        <div className="bg-white rounded-xl shadow-sm border p-4" style={{ borderColor: theme.border }}>
          <h3 className="text-sm font-semibold mb-3 flex items-center" style={{ color: theme.primary }}>
            <DollarSign className="mr-2" size={16} style={{ color: theme.accent }} />
            Budget & Requirements
          </h3>
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 text-xs">
            {/* Budget Range */}
            <div>
              <div className="text-gray-500">Budget Range:</div>
              <div className="font-bold text-sm mt-1" style={{ color: theme.accent }}>
                {formatCurrency(buyer.budget.min)} - {formatCurrency(buyer.budget.max)}
              </div>
            </div>
      ...
      {/* Progress Tracking */}
      <div className="bg-white rounded-xl shadow-sm border p-4" style={{ borderColor: theme.border }}>
        <h3 className="text-sm font-semibold mb-3" style={{ color: theme.primary }}>
          Buyer Journey Progress
        </h3>
        <div className="space-y-3 text-xs">
          {/* Overall Progress */}
          <div className="flex items-center justify-between">
            <span className="font-medium text-gray-700">Overall Progress</span>
            <span className="font-bold" style={{ color: theme.accent }}>{buyer.stageProgress}%</span>
          </div>
          ...
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div
              ...
              style={{ width: `${buyer.stageProgress || 0}%`, background: `linear-gradient(90deg, ${theme.accent} 0%, ${theme.accentLight} 100%)` }}
            ></div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border p-4" style={{ borderColor: theme.border }}>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold" style={{ color: theme.primary }}>Recent Activities</h3>
          <button
            onClick={() => setActiveTab?.('activities')}
        ...
        {buyer.activities?.length > 0 ? (
          <div className="space-y-2">
            {buyer.activities.slice(0, 3).map((activity: any) => (
              ...
              >
                <div className="p-1.5 rounded-lg" style={{ backgroundColor: `${theme.accent}15` }}>
      ...
      <div className="bg-white rounded-xl shadow-sm border p-4" style={{ borderColor: theme.border }}>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold" style={{ color: theme.primary }}>Upcoming Follow-ups</h3>
          <button
            onClick={() => setActiveTab?.('followups')}
            className="text-xs font-medium transition-colors"
            style={{ color: theme.accent }}
