...
const PropertiesTab: React.FC<PropertiesTabProps> = ({
  ...
  const reqPossession = norm(req.possession);
  const reqPropType = norm(req.propertyType);
  const reqKeywords = norm(req.specialRequirements || '').split(/\s+/).filter(Boolean);
  ...
  const getAvailabilityBadge = (status: string) => {
    ...
    const cls = map[status] || 'bg-gray-100 text-gray-700';
    return <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${cls}`}>{status || 'Available'}</span>;
  };
  ...
  const sellerFrom = (p: any) => p?.seller_name || p?.owner_name || p?.contact_name || '—';
  const sellerPhoneFrom = (p: any) => p?.seller_phone || p?.owner_phone || p?.contact_phone || p?.phone || '';
  const sizeFrom = (p: any) => {
    const unit = p?.unit_type || '';
    const area = Number(p?.carpet_area ?? p?.area ?? p?.super_builtup_area ?? 0);
    const areaTxt = area ? `${Number(area).toLocaleString('en-IN')} sq ft` : '';
  ...
  const facingFrom = (p: any) => p?.facing || p?.facing_name || p?.facing_type || p?.direction || '—';
  const parkingFrom = (p: any) => {
    const qty = p?.parking_qty ? String(p.parking_qty) : '';
    const type = p?.parking_type || '';
    return [qty, type].filter(Boolean).join(' ');
  ...
  const TagBadge: React.FC<{ label: string }> = ({ label }) => {
    ...
    const emoji = typeof tone.emoji === 'string' ? tone.emoji : null;
    return (
      <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] leading-none ring-1 ${tone.bg} ${tone.text} ${tone.ring}`} title={label}>
        {emoji ? <span className="text-[8px]">{emoji}</span> : null}
  ...
  return (
    <>
      <div className="space-y-3">
        ...
        {/* Loading / Error */}
        {(loadingProps || loadingTags) && (
          <div className="bg-white rounded-xl p-6 text-center text-xs" style={{ border: `1px solid ${BD}`, color: MU }}>
            Loading {loadingProps ? 'properties' : 'tags'}...
          </div>
        )}
        {propsError && !loadingProps && (
          <div className="bg-red-50 rounded-xl p-6 text-center text-xs text-red-600" style={{ border: `1px solid #fecaca` }}>
            {propsError}
          </div>
        )}
        {tagsError && !loadingTags && (
          <div className="bg-amber-50 rounded-xl p-6 text-center text-xs text-amber-700" style={{ border: `1px solid #fed7aa` }}>
            {tagsError}
          </div>
        )}
          ...
          <div className="rounded-lg p-2.5" style={{ background: `${O}10`, border: `1px solid ${O}20` }}>
            <div className="flex flex-wrap items-center justify-between gap-2 text-[10px]">
              <span className="font-medium" style={{ color: O }}>
                {showMatchState
                  ? `${items.length} matching ${items.length === 1 ? 'property' : 'properties'}`
                  : `${properties.length} public ${properties.length === 1 ? 'property' : 'properties'}`}
              </span>
              <span className="px-2 py-0.5 rounded-full text-[9px] font-medium" style={{ background: `${O}20`, color: O }}>
                🔓 Public listings
              </span>
            </div>
          </div>
        ...
        {showMatchState && (
          <div className="space-y-3">
            {items.map((property) => (
              <div key={property.id} className="bg-white rounded-xl overflow-hidden" style={{ border: `1px solid ${BD}` }}>
                <div className="p-3">
                  ...
                  <div className="block sm:hidden">
                    ...
                    {/* Image and Right Section - Side by Side */}
                    <div className="flex gap-3">
                      {/* Image */}
                      <div className="w-24 h-24 rounded-lg overflow-hidden flex-shrink-0" style={{ background: BG }}>
                        {property.photo ? (
                          <img src={property.photo} alt={property.title} className="w-full h-full object-cover" />
                        ) : (
                          <div className="w-full h-full flex items-center justify-center" style={{ color: MU }}>
                            <Building size={28} />
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-start justify-between gap-1">
                          <h4 className="text-sm font-semibold truncate flex-1" style={{ color: N }}>{property.title}</h4>
                          <div className="text-sm font-bold flex-shrink-0" style={{ color: O }}>{formatCurrency(property.price)}</div>
                        </div>
                        <div className="flex items-center gap-1 mt-0.5">
                          <MapPin size={10} style={{ color: MU }} />
                          <span className="text-[10px] truncate" style={{ color: MU }}>{property.address || '—'}</span>
                        </div>
                        <div className="text-[10px] mt-0.5" style={{ color: MU }}>{property.size}</div>
                        
                        {Array.isArray(property.tags) && property.tags.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1.5">
                            {property.tags.slice(0, 3).map((t, i) => (
                              <TagBadge key={`${property.id}-tag-${i}`} label={t} />
                            ))}
                            {property.tags.length > 3 && (
                              <span className="px-1.5 py-0.5 rounded-full text-[8px]" style={{ background: BG, color: MU }}>
                                +{property.tags.length - 3}
                              </span>
                            )}
                          </div>
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 mt-3">
                      <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-medium ${getMatchScoreColor(property.matchScore)}`}>
                        {property.matchScore}% Match
                      </span>
                      {getAvailabilityBadge(property.statusText)}
                      {shortlisted.has(property.id) && (
                        <span className="px-1.5 py-0.5 rounded-full text-[9px] font-medium bg-purple-100 text-purple-700">
                          ⭐ Shortlisted
                        </span>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-2 text-[10px]">
                      <div><span style={{ color: MU }}>Floor:</span> <span className="font-medium" style={{ color: N }}>{property.floorLine || '—'}</span></div>
                      <div><span style={{ color: MU }}>Facing:</span> <span className="font-medium" style={{ color: N }}>{property.facing}</span></div>
                      <div><span style={{ color: MU }}>Parking:</span> <span className="font-medium" style={{ color: N }}>{property.parking || '—'}</span></div>
                      <div><span style={{ color: MU }}>Possession:</span> <span className="font-medium" style={{ color: N }}>{property.possession}</span></div>
                    </div>

                    {/* Why it matches */}
                    {property.reasons?.length > 0 && (
                      <div className="mt-2">
                        <div className="text-[9px] font-medium mb-1" style={{ color: O }}>Why it matches:</div>
                        <div className="flex flex-wrap gap-1">
                          {property.reasons.slice(0, 3).map((r: string, i: number) => (
                            <span key={i} className="px-1.5 py-0.5 rounded-full text-[8px] font-medium" style={{ background: `${O}10`, color: O }}>
                              {r}
                            </span>
                          ))}
                        </div>
                    ...
                    {/* Seller */}
                    <div className="mt-3 pt-2" style={{ borderTop: `1px solid ${BD}` }}>
                      <div className="text-[10px]" style={{ color: MU }}>
                        <span>Seller: </span>
                        <span className="font-medium" style={{ color: N }}>{property.seller}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-2">
                      <button
                        onClick={() => onVisitClick?.(property._raw)}
                        className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-medium text-white transition-all hover:opacity-80"
                        style={{ background: O }}
                      >
                        <Calendar size={12} />
                        <span>Visit</span>
                      ...
                      <button
                        onClick={() => toggleShortlist(property.id)}
                        className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-medium transition-all hover:opacity-80"
                  ...
                  <div className="hidden sm:flex gap-3">
                    ...
                    {/* Image */}
                    <div className="w-20 h-20 rounded-lg overflow-hidden flex-shrink-0" style={{ background: BG }}>
                      {property.photo ? (
                        <img src={property.photo} alt={property.title} className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center" style={{ color: MU }}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-start justify-between gap-1">
                        <div className="flex-1 min-w-0">
                          <h4 className="text-sm font-semibold truncate" style={{ color: N }}>{property.title}</h4>
                          <div className="flex items-center gap-1 mt-0.5">
                            <MapPin size={10} style={{ color: MU }} />
                            <span className="text-[10px] truncate" style={{ color: MU }}>{property.address || '—'}</span>
                          </div>
                        ...
                        <div className="text-right flex-shrink-0">
                          <div className="text-sm font-bold" style={{ color: O }}>{formatCurrency(property.price)}</div>
                          <div className="text-[9px]" style={{ color: MU }}>{property.size}</div>
                        </div>
                      </div>

                      {Array.isArray(property.tags) && property.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1.5">
                          {property.tags.slice(0, 4).map((t, i) => (
                            <TagBadge key={`${property.id}-tag-${i}`} label={t} />
                          ))}
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-2 text-[9px]">
                        <div><span style={{ color: MU }}>Floor:</span> <span className="font-medium" style={{ color: N }}>{property.floorLine || '—'}</span></div>
                        <div><span style={{ color: MU }}>Facing:</span> <span className="font-medium" style={{ color: N }}>{property.facing}</span></div>
                        <div><span style={{ color: MU }}>Parking:</span> <span className="font-medium" style={{ color: N }}>{property.parking || '—'}</span></div>
                        <div><span style={{ color: MU }}>Possession:</span> <span className="font-medium" style={{ color: N }}>{property.possession}</span></div>
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-2 mt-3 pt-2" style={{ borderTop: `1px solid ${BD}` }}>
        ...
        {/* Empty States */}
        {showNoPublicState && (
          <div className="bg-white rounded-xl p-8 text-center" style={{ border: `1px solid ${BD}` }}>
            <Building size={40} style={{ color: MU }} className="mx-auto mb-3" />
            <h3 className="text-sm font-semibold mb-1" style={{ color: N }}>No Public Properties</h3>
            <p className="text-xs mb-4" style={{ color: MU }}>No public listings right now. Try private matches.</p>
            <button onClick={onShowPropertyMatch} className="px-4 py-1.5 rounded-lg text-white text-xs font-medium transition-all hover:opacity-80" style={{ background: O }}>
              Find Properties
            </button>
          </div>
        )}
        ...
        {showNoMatchState && (
          <div className="bg-white rounded-xl p-8 text-center" style={{ border: `1px solid ${BD}` }}>
            <Building size={40} style={{ color: MU }} className="mx-auto mb-3" />
            <h3 className="text-sm font-semibold mb-1" style={{ color: N }}>No Matching Properties</h3>
            <p className="text-xs mb-4" style={{ color: MU }}>Found {properties.length} properties, none match your criteria.</p>
            <div className="flex flex-wrap items-center justify-center gap-2">
              <button onClick={onShowPropertyMatch} className="px-4 py-1.5 rounded-lg text-white text-xs font-medium transition-all hover:opacity-80" style={{ background: O }}>
                Find Matching
              </button>
              <button onClick={onShowPropertySuggestions} className="px-4 py-1.5 rounded-lg text-white text-xs font-medium transition-all hover:opacity-80" style={{ background: N }}>
                AI Suggestions
              </button>
            </div>
          </div>
