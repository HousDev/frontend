...
interface REXVisitConfirmedCardProps {
  ...
  onBrowseMore?: () => void;
  onReschedule?: (visitData: RexVisitData) => void;
}
    ...
    <div className="my-3 w-full max-w-sm rounded-2xl bg-white border border-slate-200 shadow-xs p-4 space-y-3 text-slate-800">
      ...
      <div className="flex items-center justify-between">
        ...
        {onReschedule && (
          <button
            type="button"
            onClick={() => onReschedule(visitData)}
            className="px-2.5 py-1 text-[11px] font-semibold text-[#0f2b3d] bg-slate-100 hover:bg-slate-200 rounded-lg border border-slate-200 transition-colors cursor-pointer"
          >
            Reschedule
          </button>
        )}
        ...
        <div className="bg-white rounded-xl p-2.5 border border-slate-200 shadow-2xs flex gap-3 items-center">
          ...
          <div className="min-w-0 flex-1">
            <h4 className="text-xs font-semibold text-slate-900 truncate" title={visitData.property_title}>
              {(visitData.property_title || "Residential Property")
                .replace(/\[REX\d+\]\s*/gi, "")
                .replace(/\s*\([^)]*\)/g, "")
      ...
      <div className="space-y-1.5 pt-1">
        {onOpenChatDesk && visitData.property_id && (
          <button
            ...
            className="w-full flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl bg-[#0f2b3d] hover:bg-[#163e58] text-white font-medium text-xs shadow-2xs transition-colors cursor-pointer"
          >
