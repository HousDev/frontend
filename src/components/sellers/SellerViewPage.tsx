...
import SellerFollowupModal from './SellerFollowupModal';
  ...
  const handleSaveFollowup = (followupData: any) => {
    ...
    const updatedBuyer = {
      ...
      followups: editingFollowup
        ? (seller.followups || []).map((f: any) => (f.id === editingFollowup.id ? followupData : f))
        : [...(seller.followups || []), followupData]
    };
    onUpdateSeller(updatedBuyer);
    ...
    <div className="h-full flex flex-col bg-gray-50 text-xs">
      ...
      {showFollowupModal && (
        <SellerFollowupModal
          ...
          tabId="seller" /* <-- pass the correct master tab id so modal filters seller-specific connected remarks */
          sellerId={seller?.id ?? seller?.sellerId ?? ""} /* <-- ensure sellerId is passed */
