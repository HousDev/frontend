...
type Props = {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: SellerFollowupPayload) => void;
  tabId: string;
  leadId?: string | number | null;
  sellerId?: string | number | null;
  initialForm?: Partial<FollowupFormWithLead> & Record<string, any>;
...
const normalizeRow = (r: any): NormalizedRow => {
  ...
  const type1Name = pick(["type1Name", "type1_name", "type1", "masterType1", "master_type_1", "master_type1", "masterType1Name"]) ??
    pick(["masterType1", "master_type_1", "master_type1"]) ??
    pick(["master_type_1", "master_type_1_name"]) ??
    pick(["master_type_1"]);

  const value1Name = pick(["value1Name", "value1_name", "value1", "masterValue1", "master_value_1", "master_value1"]) ??
    pick(["master_value_1"]);

  const type2Name = pick(["type2Name", "type2_name", "type2", "masterType2", "master_type_2", "master_type2"]) ??
    pick(["master_type_2"]);

  const value2Name = pick(["value2Name", "value2_name", "value2", "masterValue2", "master_value_2", "master_value2"]) ??
    pick(["master_value_2"]);

  const tabId = pick(["masterTabId", "master_tab_id", "master_tab", "tabId", "tab_id"]) ??
    pick(["mastertabid", "master_tab"]);
  ...
  if (Array.isArray(remarksRaw)) {
    ...
    remarksArr = remarksRaw.split(/\r?\n|,/).map((x) => toStr(x).trim()).filter(Boolean);
  }
...
const uniq = (arr: string[]) => Array.from(new Set(arr.map((s) => (s || "").trim()).filter(Boolean)));
const isStageType = (typeName?: string) =>
  !!typeName && (includesKey(typeName, "sellerleadstage") || includesKey(typeName, "leadstage") || includesKey(typeName, "stage"));
const isStatusType = (typeName?: string) =>
  !!typeName && (includesKey(typeName, "sellerleadstatus") || includesKey(typeName, "leadstatus") || includesKey(typeName, "status"));
const isRemarksType = (typeName?: string) =>
  !!typeName && (includesKey(typeName, "remark") || includesKey(typeName, "remarks"));
...
/* -------- Colored Followup Type Select (Redesigned) -------- */
type TypeOption = { value: string; Icon: any; color: string };
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        disabled={disabled}
        onClick={() => setOpen((o) => !o)}
        className={`mt-1 w-full border rounded-lg px-3 py-1.5 text-left flex items-center gap-2 text-[11px]
          ...
          {options.map((opt) => {
            ...
            return (
              <button
                ...
                className={`w-full px-3 py-2 text-left flex items-center gap-2 text-[11px] transition-colors
                            ${selected ? "bg-orange-50" : "hover:bg-gray-50"}`}
              >
                <Ico size={14} className={optColorClass.split(" ")[2]} />
...
const SellerFollowupModal: React.FC<Props> = ({
  ...
  const pickFirst = <T,>(...vals: T[]) => vals.find((v) => v !== undefined && v !== null && v !== "") as T | undefined;
  const getCurrentUserId = (u: any) => pickFirst(u?.id, u?.user_id, u?._id, u?.profile?.id, u?.employee_id, u?.uid, u?.auth_id);
  const getCurrentExecutiveId = (u: any) => pickFirst(u?.executive_id, u?.employee_id, u?.profile?.executive_id, u?.currentExecutiveId, u?.agent_id);

  const currentUserId = getCurrentUserId(user);
  const currentExecutiveId = getCurrentExecutiveId(user) ?? currentUserId;

  const effectiveIdRaw = leadId ?? sellerId ?? "";
  const effectiveSellerId: string | number = typeof effectiveIdRaw === "number" ? effectiveIdRaw : String(effectiveIdRaw || "").trim();
  ...
  const resolveScheduleDate = (): string => {
    const dateRaw = readInit("scheduleDate", "schedule_date") ?? readInit("followup_date", "followup_date") ?? readInit("date", "date");
    const timeRaw = readInit("scheduleTime", "schedule_time") ?? readInit("followup_time", "followup_time") ?? readInit("time", "time");
    const combined = readInit("scheduleDateTime", "schedule_datetime") ?? readInit("datetime", "date_time");
    const d1 = parseSqlish(dateRaw);
    const d2 = parseSqlish(combined);
    return toSqlDateYYYYMMDD(d1 || d2);
  ...
  const [form, setForm] = useState<FollowupForm>(() => ({
    followupType: (readInit("followupType", "followup_type") as string) || FOLLOWUP_TYPES[0].value,
    sellerLeadStage: (readInit("sellerLeadStage", "seller_lead_stage") as string) || (readInit("buyerLeadStage", "buyer_lead_stage") as string) || "",
    sellerLeadStatus: (readInit("sellerLeadStatus", "seller_lead_status") as string) || (readInit("buyerLeadStatus", "buyer_lead_status") as string) || "",
    remark: (readInit("remark", "remark") as string) || "",
    customRemark: (readInit("customRemark", "custom_remark") as string) || (readInit("notes", "notes") as string) || "",
    nextAction: (readInit("nextAction", "next_action") as string) || "",
    scheduleDate: resolveScheduleDate(),
  ...
  useEffect(() => {
    const fetchPriorities = async () => {
      try {
        ...
        const data = await getMasterDropdownOptions(["lead"]);
        const common: any = (data as any)?.common ?? data ?? {};
        const opts: MasterOption[] = common["Lead Priority"] || common["lead priority"] || (Array.isArray((data as any)["Lead Priority"]) ? (data as any)["Lead Priority"] : []) || [];
        setPriorityOptions(Array.isArray(opts) ? opts : []);
        if (!isEdit && !initialForm?.priority && !form.priority && Array.isArray(opts) && opts.length) {
          setForm((f) => ({ ...f, priority: opts[0].value }));
        }
  ...
  useEffect(() => {
    if (!isOpen) return;
    const stageVal = (readInit("sellerLeadStage", "seller_lead_stage") as string) || (readInit("buyerLeadStage", "buyer_lead_stage") as string) || form.sellerLeadStage || "";
    if (apiRows.length && stageVal) {
      const sts = statusesForStageFrom(apiRows, stageVal);
      setAvailableStatuses(sts);
      const statusVal = (readInit("sellerLeadStatus", "seller_lead_status") as string) || (readInit("buyerLeadStatus", "buyer_lead_status") as string) || form.sellerLeadStatus || "";
      if (statusVal) {
        const rems = remarksForStatusFrom(apiRows, statusVal);
        setAvailableRemarks(rems);
      } else {
  ...
  const handleChange = (key: keyof FollowupForm) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) =>
    setForm((f) => ({ ...f, [key]: e.target.value }));
  ...
  const handleStageChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setAvailableStatuses(nextStatuses);
    setAvailableRemarks([]);
    setForm((f) => ({ ...f, sellerLeadStage: stage, sellerLeadStatus: "", remark: "", customRemark: "" }));
  };
  ...
  const handleStatusChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const nextRemarks = status ? remarksForStatusFrom(apiRows, status) : [];
    setAvailableRemarks(nextRemarks);
    setForm((f) => ({ ...f, sellerLeadStatus: status, remark: "", customRemark: "" }));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.sellerLeadStage) { alert("Please select a Seller Lead Stage."); return; }
    if (!form.sellerLeadStatus) { alert("Please select a Seller Lead Status."); return; }
    if (!effectiveSellerId) { alert("Missing seller id. Cannot save follow-up without reference."); return; }
    ...
    const payload: SellerFollowupPayload = {
      ...
      reminder: 0,
      assigned_executive: currentExecutiveId,
      ...(initialForm?.id ? { id: initialForm.id } : {}),
      created_by: initialForm?.created_by ?? currentUserId,
      created_at: initialForm?.created_at ?? nowIso,
      updated_by: currentUserId,
      updated_at: nowIso,
    };
    (payload as any).lead_id && delete (payload as any).lead_id;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4" style={{ background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(2px)' }}>
      <div className="absolute inset-0" onClick={onClose} />
      <div className="relative bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden" style={{ border: `1px solid ${BD}` }}>
        
        {/* Header */}
        <div className="px-4 sm:px-5 py-2.5 flex items-center justify-between border-b bg-[#0f2b3d]" style={{ borderColor: BD }}>
          <div className="flex items-center gap-2">
            <div className="p-1.5 rounded-lg" style={{ background: `${P}10` }}>
              <Target size={14} style={{ color: O }} />
            </div>
            <div>
              <h2 className="text-sm font-semibold text-white" >{isEdit ? "Edit Seller Follow-up" : "Add Seller Follow-up"}</h2>
              <p className="text-[10px] text-orange-600" >Schedule and track follow-up activities</p>
            </div>
          </div>
          <button onClick={onClose} className="p-1 rounded-lg hover:bg-gray-100 transition-colors"><X size={16} style={{ color: MU }} /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-4" style={{ scrollbarWidth: 'thin' }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              <FormField label="Follow-up Type " required icon={<Phone size={7} />}>
                <ColoredFollowupTypeSelect options={FOLLOWUP_TYPES as unknown as TypeOption[]} value={form.followupType} onChange={(v) => setForm((f) => ({ ...f, followupType: v }))} disabled={loading} />
              </FormField>

              <FormField label="Lead Priority" icon={<Flag size={7} />}>
                <select className="w-full border rounded-lg px-2.5 py-1.5 text-[10px] focus:outline-none focus:ring-1 bg-white" style={{ borderColor: BD }} value={form.priority} onChange={handleChange("priority")} disabled={mastersLoading || (priorityOptions.length === 0 && !form.priority)}>
                  <option value="">{mastersLoading ? "Loading..." : "Select Priority"}</option>
                  {priorityOptions.map((opt) => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}
                  {!mastersLoading && form.priority && !priorityOptions.find((o) => o.value === form.priority) && (<option value={form.priority}>{form.priority}</option>)}
                </select>
              </FormField>

              <FormField label="Seller Lead Stage" required icon={<Target size={7} />}>
                <select className="w-full border rounded-lg px-2.5 py-1.5 text-[10px] focus:outline-none focus:ring-1 bg-white" style={{ borderColor: BD }} value={form.sellerLeadStage} onChange={handleStageChange} disabled={loading || (availableStages.length === 0 && !form.sellerLeadStage)}>
                  <option value="">{loading ? "Loading..." : "Select stage…"}</option>
                  {form.sellerLeadStage && !availableStages.includes(form.sellerLeadStage) && (<option value={form.sellerLeadStage}>{form.sellerLeadStage}</option>)}
                  {availableStages.map((s) => (<option key={s} value={s}>{s}</option>))}
                </select>
              </FormField>

              <FormField label="Seller Lead Status" required icon={<UserCheck size={7} />}>
                <select className="w-full border rounded-lg px-2.5 py-1.5 text-[10px] focus:outline-none focus:ring-1 bg-white" style={{ borderColor: BD }} value={form.sellerLeadStatus} onChange={handleStatusChange} disabled={loading || !form.sellerLeadStage || (availableStatuses.length === 0 && !form.sellerLeadStatus)}>
                  <option value="">{form.sellerLeadStage ? "Select status…" : "Select stage first"}</option>
                  {form.sellerLeadStatus && !availableStatuses.includes(form.sellerLeadStatus) && (<option value={form.sellerLeadStatus}>{form.sellerLeadStatus}</option>)}
                  {availableStatuses.map((s) => (<option key={s} value={s}>{s}</option>))}
                </select>
              </FormField>

              <FormField label="Remarks" icon={<FileText size={7} />}>
                <select className="w-full border rounded-lg px-2.5 py-1.5 text-[10px] focus:outline-none focus:ring-1 bg-white" style={{ borderColor: BD }} value={form.remark} onChange={handleRemarkChange} disabled={loading || !form.sellerLeadStatus || (availableRemarks.length === 0 && !form.remark)}>
                  <option value="">{form.sellerLeadStatus ? "Select remark…" : "Select status first"}</option>
                  {form.remark && !availableRemarks.includes(form.remark) && (<option value={form.remark}>{form.remark}</option>)}
                  {availableRemarks.map((r) => (<option key={r} value={r}>{r}</option>))}
                </select>
              </FormField>

              <FormField label="Next Action" icon={<Target size={7} />}>
                <input type="text" className="w-full border rounded-lg px-2.5 py-1.5 text-[10px] focus:outline-none focus:ring-1 bg-white" style={{ borderColor: BD }} placeholder="Enter next action..." value={form.nextAction} onChange={handleChange("nextAction")} />
              </FormField>
            </div>

            <FormField label="Custom Remark" icon={<FileText size={7} />}>
              <textarea className="w-full border rounded-lg px-2.5 py-1.5 text-[10px] focus:outline-none focus:ring-1 bg-white resize-none" style={{ borderColor: BD }} rows={2} placeholder="Select a remark above or type your own details…" value={form.customRemark} onChange={handleChange("customRemark")} />
            </FormField>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              <FormField label="Schedule Date" required icon={<Calendar size={7} />}>
                <input type="date" className="w-full border rounded-lg px-2.5 py-1.5 text-[10px] focus:outline-none focus:ring-1 bg-white" style={{ borderColor: BD }} value={form.scheduleDate} onChange={handleChange("scheduleDate")} min={dateMin} />
              </FormField>
              <FormField label="Schedule Time" required icon={<Clock size={7} />}>
                <input type="time" className="w-full border rounded-lg px-2.5 py-1.5 text-[10px] focus:outline-none focus:ring-1 bg-white" style={{ borderColor: BD }} value={form.scheduleTime} onChange={handleChange("scheduleTime")} />
              </FormField>
            </div>

            {/* Footer Actions */}
            <div className="flex flex-col sm:flex-row justify-end gap-2 pt-3 border-t" style={{ borderColor: BD }}>
              <button type="button" onClick={onClose} className="px-3 py-1.5 text-[9px] font-medium rounded-lg transition-all hover:bg-gray-50 order-2 sm:order-1" style={{ border: `1px solid ${BD}`, color: N }}>Cancel</button>
              <button type="submit" disabled={loading} className="flex items-center justify-center gap-1 px-3 py-1.5 text-[9px] font-medium text-white rounded-lg transition-all hover:opacity-90 disabled:opacity-50 order-1 sm:order-2" style={{ background: P }}>
                {loading ? <><span className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin" /> {isEdit ? "Updating..." : "Saving..."}</> : <>{isEdit ? "Update Follow-up" : "Save Follow-up"}</>}
              </button>
            </div>
          </form>
        </div>
