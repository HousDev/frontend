...
const statusesForStageFrom = (rows: Row[], stage: string) => {
    ...
    rows.forEach((r) => {
        const left =
            r.type1Name === "Seller Lead Stage" &&
            r.value1Name === stage &&
            r.type2Name === "Seller Lead Status";
        const right =
            r.type2Name === "Seller Lead Stage" &&
            r.value2Name === stage &&
            r.type1Name === "Seller Lead Status";
...
const remarksForStatusFrom = (rows: Row[], status: string) => {
    ...
    rows.forEach((r) => {
        const statusMatches =
            (r.type1Name === "Seller Lead Status" && r.value1Name === status) ||
            (r.type2Name === "Seller Lead Status" && r.value2Name === status);

        if (statusMatches && Array.isArray(r.remarks) && r.remarks.length) {
            out.push(...r.remarks.filter(Boolean));
        }
        // handle pairs where Remarks is provided as valueXName
        if (r.type1Name === "Seller Lead Status" && r.value1Name === status && r.type2Name === "Remarks") {
            out.push(r.value2Name);
        }
        if (r.type2Name === "Seller Lead Status" && r.value2Name === status && r.type1Name === "Remarks") {
...
const SellerFollowupModal: React.FC<Props> = ({ isOpen, onClose, onSave, tabId, leadId, sellerId, initialForm }) => {
    ...
    const [form, setForm] = useState<FollowupForm>(() => ({
        ...
        sellerLeadStage: initialForm?.sellerLeadStage || "",
        sellerLeadStatus: initialForm?.sellerLeadStatus || "",
    ...
    useEffect(() => {
        const load = async () => {
            ...
            try {
                ...
                // Extract Seller Lead Stage values from master
                const stages = extractAllOfTypeFrom(safe, "Seller Lead Stage");
                setAvailableStages(stages);

                // For edit: hydrate statuses/remarks chains
                if (initialForm?.sellerLeadStage) {
                    const sts = statusesForStageFrom(safe, initialForm.sellerLeadStage);
                    setAvailableStatuses(sts);
                    if (initialForm.sellerLeadStatus) {
                        const rems = remarksForStatusFrom(safe, initialForm.sellerLeadStatus);
    ...
    useEffect(() => {
        ...
        // hydrate dependent dropdowns if apiData already loaded
        if (apiData.length && initialForm?.sellerLeadStage) {
            const sts = statusesForStageFrom(apiData, initialForm.sellerLeadStage);
            setAvailableStatuses(sts);
            const rems = initialForm.sellerLeadStatus ? remarksForStatusFrom(apiData, initialForm.sellerLeadStatus) : [];
            setAvailableRemarks(rems);
        } else {
            setAvailableStatuses([]);
            setAvailableRemarks([]);
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select
                                ...
                                disabled={loading || !form.sellerLeadStage || availableStatuses.length === 0}
                            >
                        ...
                        <div>
                            ...
                            <select
                                ...
                                disabled={loading || !form.sellerLeadStage || !form.sellerLeadStatus || availableRemarks.length === 0}
                            >
