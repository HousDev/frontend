...
export default function CampaignDetailModal({ campaign, onClose, onLaunch }: Props) {
    // ✅ useCampaignLogs hook now uses real API via whatsappAPI
    const { logs, loading, refresh } = useCampaignLogs(campaign.id);
                    ...
                    <div>
                        ...
                        {loading ? (
                            ...
                            <div className="text-center py-8 bg-gray-50 rounded-xl border border-gray-100">
                                <p className="text-sm text-gray-400">No delivery logs yet</p>
                                <p className="text-xs text-gray-400 mt-1">Logs will appear after campaign launch</p>
                            </div>
                        ) : (
                            <div className="space-y-2 max-h-64 overflow-y-auto">
                                {logs.map((log) => {
                                    const statusConfig: Record<string, { icon: React.ElementType; color: string; bg: string }> = {
                                        ...
                                        read: { icon: CheckCircle, color: 'text-purple-600', bg: 'bg-purple-50' },
                                    ...
                                    return (
                                        <div key={log.id} className={`flex items-center justify-between p-3 rounded-lg border ${config.bg} border-gray-100`}>
                                            <div className="flex items-center gap-3 min-w-0">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="text-xs text-gray-400">{log.contact?.phone || 'No phone'}</p>
                                                </div>
                                            </div>
                                            <div className="text-right shrink-0 ml-2">
                                                <p className={`text-xs font-semibold uppercase ${config.color}`}>{log.status}</p>
                                                {log.sent_at && (
