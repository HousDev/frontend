...
function getAvatarColor(name: string): string {
  const colors = [
    ...
    '#FFB74D', '#FF8A65', '#A1887F', '#90A4AE', '#DCE775', '#FFF176'
  ];
...
export default function ConversationList({
  ...
  const [pendingFilter, setPendingFilter] =
    useState<AdvancedFilter>(emptyAdvanced);
  ...
  const hasActiveFilter =
    advanced.datePreset !== null ||
    advanced.dateFrom ||
    advanced.dateTo ||
    advanced.stages.length > 0 ||
    advanced.hasUnread !== null ||
    advanced.tagSearch;
...
const filteredByTab = conversations.filter((conv) => {
  if (filter === 'unread') return Number(conv.unread_count) > 0;
  if (filter === 'assigned') return conv.assigned_to && conv.assigned_to !== 'Unassigned' && conv.assigned_to !== '';
  if (filter === 'buyer') {
...
const filteredConversations = filteredByTab.filter((conv) => {
   ...
   // ✅ FIXED
if (advanced.hasUnread === true && Number(conv.unread_count) <= 0) return false;
if (advanced.hasUnread === false && Number(conv.unread_count) > 0) return false;
    if (advanced.tagSearch) {
      const tags = conv.contact?.tags || [];
      const match = tags.some((t) =>
        t.name.toLowerCase().includes(advanced.tagSearch.toLowerCase()),
      );
      if (!match) return false;
    }
    if (advanced.datePreset || advanced.dateFrom || advanced.dateTo) {
      const msgDate = conv.last_message_at
      ...
      if (advanced.dateFrom && msgDate < new Date(advanced.dateFrom))
        return false;
      if (advanced.dateTo) {
      ...
      <div className="px-4 pt-4 pb-3 border-b border-gray-100">
        ...
        <div className="flex gap-1 overflow-x-auto whitespace-nowrap scrollbar-hide">
          ...
          <button
            ...
            className={`px-2 py-1 text-[11px] font-medium rounded-md transition-all shrink-0 ${
              filter === "all"
          ...
          <button
            ...
            className={`px-2 py-1 text-[11px] font-medium rounded-md transition-all flex items-center gap-1 shrink-0 ${
              filter === "unread"
                ? "bg-emerald-600 text-white"
                : "bg-gray-100 text-gray-600 hover:bg-gray-200"
            }`}
        ...
        {loading && filteredConversations.length === 0 ? (
          ...
          filteredConversations.map((conv) => {
            ...
            const priorityTag = tags.find((t) =>
              PRIORITY_TAGS.includes(t.name),
            );
            const otherTags = tags
              .filter((t) => !PRIORITY_TAGS.includes(t.name))
              .slice(0, 1);
            ...
            return (
              <button
                ...
                className={`w-full flex items-start gap-3 px-4 py-3 border-b border-gray-50 text-left transition-all ${
                  isSelected
  ...
  <span className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-white ${STAGE_DOT[stage] || 'bg-gray-400'}`} />
</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-1 mb-0.5">
                    <span
                      className={`text-sm font-semibold truncate leading-tight ${hasUnread ? "text-gray-900" : "text-gray-700"}`}
                    >
                      {displayName}
                    </span>
                    <span className="text-[10px] text-gray-400 shrink-0 mt-0.5 leading-tight">
                      {formatRelativeTime(conv.last_message_at)}
                    </span>
                  </div>
                  <p
                    className={`text-xs leading-tight truncate mb-1 ${hasUnread ? "text-gray-700 font-medium" : "text-gray-400"}`}
                  >
                    ...
                    <div className="flex items-center gap-1 flex-wrap">
                      {priorityTag && (
                        <span
                          ...
                          style={{
                            backgroundColor: priorityTag.color + "22",
      ...
      {showFilter && (
        <FilterModal
          ...
          onClear={() => {
            setPendingFilter(emptyAdvanced);
            applyAdvanced(emptyAdvanced);
          }}
...
function FilterModal({
  ...
  const toggleStage = (s: string) => {
    onChange({
      ...value,
      stages: value.stages.includes(s)
        ? value.stages.filter((x) => x !== s)
        : [...value.stages, s],
    });
  };
            ...
            {DATE_PRESETS.map((p) => (
              <button
                ...
                onClick={() =>
                  onChange({
                    ...value,
                    datePreset: value.datePreset === p ? null : p,
                    dateFrom: "",
                    dateTo: "",
                  })
                }
        ...
        <FilterSection title="Filter by Tag">
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) =>
                onChange({ ...value, tagSearch: e.target.value })
              }
