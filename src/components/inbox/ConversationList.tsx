...
// import { Search, RefreshCw, MessageSquare, SlidersHorizontal, X, Calendar, CheckCircle, Bell } from 'lucide-react';
...
export default function ConversationList({
    ...
    // Store all conversations when filter is 'all'
    useEffect(() => {
        if (conversations.length > 0 && filter === 'all') {
            setAllConversations(conversations);
        }
    ...
    useEffect(() => {
        ...
        socket.on("new_notification", (data) => {
            ...
            notificationStore.push(
                ...
                data.body,
                { label: 'View', page: 'inbox', contact_id: data.reference_id || data.contact_id } as any
            );
        });
    ...
    const totalCount = allConversations.length;
    const unreadCount = allConversations.filter(c => c.unread_count > 0).length;
    const assignedCount = allConversations.filter(c => c.assigned_to && c.assigned_to !== '' && c.assigned_to !== 'null').length;
    const buyerCount = allConversations.filter(c => {
        const tags = c.contact?.tags || [];
        return tags.some((t: any) => t.name?.toLowerCase() === 'buyer');
    }).length;
