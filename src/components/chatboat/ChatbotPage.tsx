...
import {
    ...
    Save, X, ChevronDown, ChevronUp, ArrowRight, MessageSquare, HelpCircle,
    List, Tag as TagIcon, UserCheck, TrendingUp, FileText, CheckCircle,
    Hash, Eye, Loader2, Download, Upload, FileJson, AlertCircle, Check,
    Square, CheckSquare,
} from 'lucide-react';
import type { ChatbotFlow, FlowStep, FlowStepType, Template, Tag, CrmUser } from '../../types';
import { whatsappAPI } from '../../lib/whatsappApi';
...
export default function ChatbotPage() {
    ...
    const fetchFlows = useCallback(async () => {
        setLoading(true);
        try {
            const data = await whatsappAPI.getChatbotFlows();
            // Cast through any to bypass type checking
    ...
    const fetchTemplates = async () => {
        try {
            ...
            setTemplates(data.filter((t: any) => t.status === 'APPROVED') as any);
        } catch (error) {
            console.error('Failed to fetch templates:', error);
        }
    };
    ...
    const fetchTags = async () => {
        try {
            const data = await whatsappAPI.getTags();
            // Convert tag IDs to string if needed
            const convertedTags = data.map((tag: any) => ({
                ...tag,
                id: String(tag.id)
            }));
            setTags(convertedTags as any);
        } catch (error) {
            console.error('Failed to fetch tags:', error);
        }
    };
    ...
    const handleDownloadSampleData = useCallback(() => {
        const sampleData = {
            ...
            flows: [
                {
                    id: "sample_flow_001",
                    name: "Property Assistant Flow",
                    description: "WhatsApp chatbot flow for property inquiries - Buy, Sell, Rent, or Talk to Expert",
                    ...
                    steps: [
                        {
                            ...
                            step_index: 0,
                            step_type: "message",
                            message_text: "Hi {{1}} 👋\n\nWelcome to our property assistance service! 🏡\n\nPlease choose an option:\n\n1️⃣ Buy Property\n2️⃣ Sell Property\n3️⃣ Rent Property\n4️⃣ Talk to Expert\n\nReply with 1, 2, 3 or 4 👇",
                            next_step_index: 1,
                            conditions: null,
                            save_response_as: null,
                            buttons: null,
                        ...
                        },
                        {
                            ...
                            step_index: 4,
                            step_type: "question",
                            message_text: "Awesome 👌\n\nPlease share:\n• Property Details\n• Location\n• Expected Price",
                            next_step_index: 5,
                            conditions: null,
                            save_response_as: "sell_details",
                            buttons: null,
                            tag_id: null,
                            assign_to: null,
                            stage: null,
                        ...
                        },
                        {
                            ...
                            step_index: 7,
                            step_type: "message",
                            message_text: "✅ Thank you!\n\nWe will share matching rental properties soon.",
                            next_step_index: null,
                            conditions: null,
                            save_response_as: null,
                            buttons: null,
        ...
        a.href = url;
        a.download = `chatbot-sample-data-${new Date().toISOString().slice(0, 19)}.json`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);

        notificationStore.push('success', 'Sample Data Downloaded', 'Sample flow data has been downloaded');
    }, []);
    ...
    const handleExportFlows = useCallback(async () => {
        try {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `chatbot-flows-${new Date().toISOString().slice(0, 19)}.json`;
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            URL.revokeObjectURL(url);

            notificationStore.push('success', 'Export Successful', `${data.length} flows exported`);
        } catch (error) {
            console.error('Failed to export flows:', error);
            notificationStore.push('error', 'Export Failed', 'Could not export flows');
        }
    }, []);
    ...
    const handleFileUpload = useCallback((file: File) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                const flowsToImport = importData.flows || (Array.isArray(importData) ? importData : [importData]);

                const previewWithStatus = flowsToImport.map((flow: any, index: number) => {
                    const existingFlow = flows.find(f => f.name === flow.name);
                    return {
                        ...flow,
                        _previewIndex: index,
                        _exists: !!existingFlow,
                        _existingId: existingFlow?.id,
                        _selected: !existingFlow,
                    };
                });
                ...
                notificationStore.push('error', 'Invalid File', 'Please upload a valid JSON file');
            }
        };
    ...
    const toggleAllSelection = () => {
        if (selectedFlows.size === importPreview.length) {
            setSelectedFlows(new Set());
        } else {
            setSelectedFlows(new Set(importPreview.map((_: any, i: number) => i)));
        }
    };
    ...
    const handleImportSelected = async () => {
        ...
        for (const [index, flow] of importPreview.entries()) {
            if (!selectedFlows.has(index)) {
                skipCount++;
                continue;
            }

            try {
                const { id, created_at, updated_at, _previewIndex, _exists, _existingId, _selected, ...flowData } = flow;

                const stepsWithNewIds = (flowData.steps || []).map((step: any) => ({
                    ...step,
                    id: undefined,
                    flow_id: undefined,
                }));

                await whatsappAPI.createChatbotFlow({
                    ...flowData,
                    steps: stepsWithNewIds,
                    is_active: false,
                });
                successCount++;
            } catch (err) {
                console.error('Failed to import flow:', flow.name, err);
                failCount++;
            }
        }
        ...
        notificationStore.push(
            'success',
            'Import Complete',
            `${successCount} imported, ${skipCount} skipped, ${failCount} failed`
        );
    ...
    const handleToggleActive = async (flow: ChatbotFlow) => {
        try {
            const updated = await whatsappAPI.toggleChatbotFlow(flow.id, !flow.is_active);
            setFlows(prev => prev.map(f => f.id === flow.id ? updated as any : f));
            notificationStore.push('success', 'Flow Updated', `${flow.name} is now ${updated.is_active ? 'active' : 'inactive'}`);
        } catch (error) {
            console.error('Failed to toggle flow:', error);
            notificationStore.push('error', 'Failed', 'Could not update flow status');
    ...
    const handleEdit = async (flow: ChatbotFlow) => {
        try {
            const fullFlow = await whatsappAPI.getChatbotFlowById(flow.id);
            setEditingFlow(fullFlow as any);
            setShowBuilder(true);
        } catch (error) {
            console.error('Failed to fetch flow details:', error);
            notificationStore.push('error', 'Failed', 'Could not load flow details');
    ...
    const handlePreview = async (flow: ChatbotFlow) => {
        try {
            const fullFlow = await whatsappAPI.getChatbotFlowById(flow.id);
            setPreviewFlow(fullFlow as any);
        } catch (error) {
            console.error('Failed to fetch flow details:', error);
            notificationStore.push('error', 'Failed', 'Could not load flow preview');
        }
        ...
        <div className="flex flex-col h-full bg-gray-50">
            ...
            <div className="bg-white border-b border-gray-200 px-3 sm:px-6 py-3 sm:py-4 flex items-center justify-between shrink-0">
                ...
                <div className="flex items-center gap-2 shrink-0">
                    <button
                        ...
                        className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 bg-gray-600 text-white text-xs sm:text-sm font-semibold rounded-xl hover:bg-gray-700 transition-colors"
                        title="Export all flows"
                    >
                        <Download size={14} />
            ...
            {showImportModal && (
                <ImportModal
                    ...
                    onToggleFlow={toggleFlowSelection}
                    ...
                    onDownloadSample={handleDownloadSampleData}
                />
            )}
...
function ImportModal({
    ...
    onToggleFlow: (index: number) => void;
    onToggleAll: () => void;
    onImport: () => void;
    onClose: () => void;
    ...
    const hasPreview = importPreview.length > 0;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                    <div>
                        <h2 className="font-bold text-gray-900 text-lg">Import Chatbot Flows</h2>
                        <p className="text-sm text-gray-500">Upload a JSON file to import flows</p>
                    </div>
                    <button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500">
                        <X size={20} />
                    </button>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    {!hasPreview ? (
                        <div>
                            {/* Download Sample Button at the top of empty state */}
                            <div className="mb-6">
                                <button
                                    onClick={onDownloadSample}
                                    className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-blue-600 text-white text-sm font-semibold rounded-xl hover:bg-blue-700 transition-colors"
                                >
                                    <FileJson size={18} />
                                    Download Sample JSON File
                                </button>
                                <p className="text-xs text-center text-gray-500 mt-2">
                                    Get a sample file to understand the format
                                </p>
                            </div>
                            ...
                            <div className="relative">
                                ...
                                <div className="relative flex justify-center text-xs uppercase">
                                    <span className="bg-white px-2 text-gray-500">Or upload your file</span>
                                </div>
                            </div>

                            <div
                                className={`border-2 border-dashed rounded-xl p-8 text-center transition-all cursor-pointer mt-6
                            ...
                            >
                                ...
                                <p className="text-lg font-semibold text-gray-700 mb-2">Upload JSON File</p>
                                <p className="text-sm text-gray-500 mb-4">Drag and drop your JSON file here, or click to browse</p>
                                ...
                                <p className="text-xs text-gray-400 mt-4">File should be exported from Chatbot Flows</p>
                            </div>
                        </div>
                    ) : (
                        <>
                            <div className="flex items-center justify-between mb-4 pb-2 border-b border-gray-100">
                                <div className="flex items-center gap-3">
                                    ...
                                    >
                                        {selectedFlows.size === importPreview.length ? (
                                            <CheckSquare size={16} className="text-emerald-600" />
                                        ) : (
                                            <Square size={16} />
                                        )}
                                        {selectedFlows.size === importPreview.length ? 'Deselect All' : 'Select All'}
                                    </button>
                                    <span className="text-sm text-gray-500">
                                        {selectedFlows.size} of {importPreview.length} selected
                                    </span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <button
                            ...
                            <div className="space-y-3 max-h-96 overflow-y-auto">
                                {importPreview.map((flow, idx) => {
                                    const isSelected = selectedFlows.has(idx);
                                    const exists = flow._exists;
                                    const stepCount = flow.steps?.length || 0;

                                    return (
                                        <div
                                            key={idx}
                                            className={`border rounded-xl p-4 transition-all cursor-pointer ${isSelected ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-gray-300'}`}
                                            onClick={() => onToggleFlow(idx)}
                                        >
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex items-center gap-2 flex-wrap mb-1">
                                                        ...
                                                        {exists && (
                                                            <span className="text-xs px-2 py-0.5 bg-red-50 text-red-600 rounded-full flex items-center gap-1">
                    ...
                    {hasPreview && (
                        ...
                        >
                            {importing ? (
                                ...
                                <>
                                    <Upload size={14} />
                                    Import {selectedFlows.size} Flow{selectedFlows.size !== 1 ? 's' : ''}
                                </>
                            )}
                        </button>
                    )}
        ...
        <div className="border border-gray-200 rounded-xl overflow-hidden">
            <button onClick={onToggle} className="w-full flex items-center gap-3 px-4 py-3 bg-gray-50 hover:bg-gray-100 transition-colors text-left">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {step.message_text && (
                        <span className="text-xs text-gray-400 ml-2 truncate hidden sm:inline">— {step.message_text.substring(0, 40)}</span>
                    )}
                </div>
                <div className="flex items-center gap-1 shrink-0">
                    {idx > 0 && (
