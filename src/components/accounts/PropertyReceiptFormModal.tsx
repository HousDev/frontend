...
const PropertyReceiptFormModal = ({ isOpen, onClose, receipt, onSave, userRole }: any) => {
  ...
  // data sources (same flow as invoice)
  ...
  useEffect(() => {
    const fetchMasters = async () => {
      try {
        ...
        // "common" tab me "Banks" and "Payment Types" defined hain
  ...
  // INIT from receipt only after data is ready
  useEffect(() => {
    if (!isOpen) return;
    if (prefilledRef.current) return;
    if (isLoadingData || masterLoading) return; // ⛔ wait till lists ready
    if (!receipt) return;
  ...
  }, [isOpen, isLoadingData, masterLoading, receipt, user?.id]);

  // reset flag when modal closes
  useEffect(() => { if (!isOpen) prefilledRef.current = false; }, [isOpen]);
  // --- options (sorted ascending by label) ---
  const bankOptions: MasterOption[] = masters?.banks ?? [];
  const paymentTypeOptions: MasterOption[] = masters?.['payment types'] ?? [];
  const paymentMethodRaw: MasterOption[] =
    masters?.['payement methods'] || masters?.['payment methods'] || [];
  ...
  // state (extended to include *_id like invoice)
  const [formData, setFormData] = useState({
    ...
    // optional deal value (auto from property if available)
    deal_value: 0, // kept (unused in UI but useful context)
...
payment_date: toLocalDatetimeInput(new Date()),
    ...
    transaction_details: {
      payment_method: '',
      buyer_bank_name: '', // <-- NO DEFAULT
      seller_bank_name: '', // <-- NO DEFAULT
    },
  ...
  // IMPORTANT: remove earlier auto-defaulting effect.
  // We DO NOT set any default payment_type or bank names automatically.
  ...
  // ---------- data load (same style as invoice) ----------
  useEffect(() => {
    if (!isOpen) return;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {/* Payment Method (from master "Payement Methods") */}
                    <div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <button
                ...
                disabled={
                  ...
                  !formData.payment_type ||                 // must select
                  !formData.amount ||
...
export default PropertyReceiptFormModal;
