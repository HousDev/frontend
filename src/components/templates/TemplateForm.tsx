...
import {
    ...
    ChevronRight, ChevronDown, Upload, MapPin, LayoutGrid, Tag, Phone, Link,
...
// <div className="w-full sm:w-44 border-r ... shrink-0 sm:border-b-0 border-b mb-2 sm:mb-0">                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-wider px-4 py-2">Categories</p>
...
function ExploreLibrary({
    ...
    const currentCategoryLabel = SIDEBAR_ITEMS.find(item => item.id === suggestionFilter)?.label || 'All Templates';
    const currentIcon = SIDEBAR_ITEMS.find(item => item.id === suggestionFilter)?.icon;

    return (
        <div className="flex flex-1 min-h-0 overflow-hidden">
            {/* Desktop sidebar - unchanged */}
            <div className="hidden sm:flex sm:w-44 border-r border-gray-100 shrink-0 bg-gray-50/50 overflow-y-auto flex-col">
            ...
            {/* Main content area */}
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                {/* Search bar - on mobile we also add a dropdown button above it */}
                <div className="px-4 py-2 sm:px-4 sm:py-3 border-b border-gray-100 shrink-0">
                    {/* Mobile category dropdown (only visible on small screens) */}
                    <div className="sm:hidden mb-2 relative">
                        <button
                            onClick={() => setMobileDropdownOpen(!mobileDropdownOpen)}
                            className="w-full flex items-center justify-between px-3 py-2 bg-white border border-gray-200 rounded-xl text-sm text-gray-700"
                        >
                            <div className="flex items-center gap-2">
{currentIcon && (() => {
    const IconComponent = currentIcon;
    return <IconComponent size={14} className="text-gray-500" />;
                                ...
                                <span className="text-[10px] text-gray-400 bg-gray-100 px-1.5 rounded-full">
                                    {SUGGESTIONS.filter((s) => suggestionFilter === 'all' || s.group === suggestionFilter).length}
                                </span>
                            </div>
                        ...
                        {mobileDropdownOpen && (
                            <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-30 overflow-hidden">
                                {SIDEBAR_ITEMS.map((item) => {
                                    const Icon = item.icon;
                                    const isActive = suggestionFilter === item.id;
                                    const count = SUGGESTIONS.filter((s) => item.id === 'all' || s.group === item.id).length;
                                    return (
                                        <button
                                            key={item.id}
                                            onClick={() => {
                                                setSuggestionFilter(item.id);
                                                setMobileDropdownOpen(false);
                                            }}
                                            className={`w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors ${isActive ? 'bg-emerald-50 text-emerald-700' : 'text-gray-700 hover:bg-gray-50'
                                                }`}
                                        >
                                            <Icon size={14} className={isActive ? 'text-emerald-600' : 'text-gray-400'} />
                    ...
                    {/* Search input - same for both mobile and desktop */}
                    <div className="relative">
                        <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                        <input
                ...
                {/* Template cards - unchanged layout */}
                <div className="flex-1 overflow-y-auto p-4">
                    {filteredSuggestions.length === 0 ? (
                        <div className="text-center py-12 text-gray-400">
                            <Lightbulb size={24} className="mx-auto mb-2 opacity-30" />
...
export default function TemplateForm({ template, onSubmit, onClose }: Props) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 sm:p-4 ">
  <div className="bg-white rounded-2xl shadow-2xl w-full sm:max-w-3xl max-h-[90vh] sm:max-h-[93vh] flex flex-col overflow-hidden">
...
<div className="flex items-center justify-between px-3 sm:px-6 py-3 sm:py-4 border-b border-gray-100 shrink-0">
                ...
                <div className="flex border-b border-gray-100 px-2 sm:px-6 shrink-0 bg-gray-50/50">
  {(['explore', 'edit', 'preview'] as const).map((tab) => (
                        <button
                            key={tab}
                            onClick={() => setActiveTab(tab)}
                            className={`flex items-center gap-1 sm:gap-1.5 px-2 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm font-medium border-b-2 capitalize transition-colors ${activeTab === tab ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-gray-400 hover:text-gray-600'
                                }`}
                        >
                ...
                <div className="flex flex-1 min-h-0 relative">
                    ...
                    {activeTab === 'edit' && (
                        <div className="flex flex-1 min-h-0">
                            <div className={`flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-5 ${isApproved ? 'opacity-80 pointer-events-none select-none' : ''}`}>
...
<div className="flex flex-row sm:flex-row items-center justify-between gap-2 px-3 sm:px-6 py-3 sm:py-4 border-t border-gray-100 shrink-0 bg-gray-50/50 overflow-x-auto">

  {/* Left section */}
  <div className="flex items-center gap-2 flex-nowrap">
    {cat && (
      <span className={`text-xs px-2 py-1 rounded-full border font-medium whitespace-nowrap ${cat.color}`}>
        {cat.label}
      </span>
    )}

    <span className="text-xs px-2 py-1 rounded-full border border-gray-200 bg-gray-50 text-gray-500 font-medium whitespace-nowrap">
      {form.template_type}
    </span>

    {variables.length > 0 && (
      <span className="text-xs text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200 whitespace-nowrap">
  ...
  <div className="flex items-center gap-2 flex-nowrap">
    <button
      onClick={onClose}
      className="px-3 py-1.5 text-xs sm:text-sm text-gray-600 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors whitespace-nowrap"
    >
      Cancel
    </button>
    ...
    {isApproved ? (
      <button
        disabled
        className="flex items-center gap-1 px-3 py-1.5 bg-emerald-100 text-emerald-700 text-xs sm:text-sm font-semibold rounded-xl opacity-60 cursor-not-allowed whitespace-nowrap"
      >
      ...
      <>
        ...
        <button
          ...
          className="flex items-center gap-1 px-3 py-1.5 text-xs sm:text-sm font-semibold border border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 disabled:opacity-50 transition-colors whitespace-nowrap"
        >
          <Save size={12} />
          {saving ? 'Saving...' : 'Save Draft'}
        </button>

        {/* Submit to Meta (Added back) */}
        {/* <button
          onClick={handleSubmitToMeta}
          disabled={saving || !form.name.trim() || !form.body.trim()}
          className="flex items-center gap-1 px-3 py-1.5 bg-emerald-600 text-white text-xs sm:text-sm font-semibold rounded-xl hover:bg-emerald-700 disabled:opacity-50 transition-colors shadow-sm whitespace-nowrap"
        >
          <Send size={12} />
