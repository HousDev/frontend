...
const [bulkDeleteConfirm, setBulkDeleteConfirm] = useState(false);
    // Add this Toast component after all imports
const Toast = ({ message, type, onClose }: { message: string; type: 'success' | 'error' | 'warning'; onClose: () => void }) => {
    ...
    const icon = type === 'success' ? <CheckCircle size={16} /> : type === 'error' ? <XCircle size={16} /> : <AlertTriangle size={16} />;

    return (
        <div className={`fixed top-4 right-4 z-50 flex items-center gap-2 px-4 py-3 rounded-xl shadow-lg text-white ${bgColor} animate-in slide-in-from-right`}>
            {icon}
    ...
    // };
        ...
        try {
            ...
            if (result.templates) {
                setTemplates(result.templates);
                setLastSync(new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }));
                setToast({ message: `Synced ${result.updated || 0} template status updates!`, type: 'success' });
            }
        } catch (err) {
            console.error('Sync failed:', err);
            setToast({ message: 'Failed to sync template status', type: 'error' });
        } finally {
            setSyncing(false);
        }
    };
    ...
    // };
        ...
        try {
            ...
            if (!editing) {
                const existingTemplate = templates.find(t => t.name === data.name);
                if (existingTemplate) {
                    setToast({ message: `Template name "${data.name}" already exists!`, type: 'error' });
                    return;
                }
            } else {
                // Check for duplicate name when editing (excluding current template)
                const existingTemplate = templates.find(t => t.name === data.name && t.id !== editing.id);
                if (existingTemplate) {
                    setToast({ message: `Template name "${data.name}" already exists!`, type: 'error' });
                    return;
                }
            ...
            setToast({ 
                message: err?.response?.data?.message || `Failed to ${editing ? 'update' : 'create'} template`, 
                type: 'error' 
            });
        }
    };
    ...
    // };
        ...
        try {
            ...
            setToast({ message: err?.response?.data?.message || 'Failed to delete template', type: 'error' });
        }
    };

    // Handle select/deselect single template
// Handle select/deselect single template
const handleSelectTemplate = (id: number | string) => {
    const numId = Number(id);
    setSelectedTemplates((prev: Set<number>) => {
...
const handleSelectAll = () => {
    if (selectedTemplates.size === filtered.length) {
        setSelectedTemplates(new Set<number>());
    } else {
        setSelectedTemplates(new Set<number>(filtered.map(t => Number(t.id))));
    }
};
...
const handleBulkDelete = async () => {
    try {
        const deletePromises = Array.from(selectedTemplates).map(id => 
            whatsappAPI.deleteTemplate(id.toString())
        );
        await Promise.all(deletePromises);
        await fetchTemplates();
        setSelectedTemplates(new Set());
        setBulkDeleteConfirm(false);
        setToast({ message: `${selectedTemplates.size} templates deleted successfully!`, type: 'success' });
    } catch (err: any) {
        console.error('Failed to delete templates', err);
        setToast({ message: err?.response?.data?.message || 'Failed to delete templates', type: 'error' });
    }
};
    // const handleDuplicate = async (t: Template) => {
    //     const { id: _id, created_at: _ca, updated_at: _ua, ...rest } = t;
    ...
    // };
        ...
        try {
            ...
            const existingTemplate = templates.find(template => template.name === newName);
            if (existingTemplate) {
                setToast({ message: `Template "${newName}" already exists! Try a different name.`, type: 'error' });
                return;
            }
            
            const { id: _id, created_at: _ca, updated_at: _ua, ...rest } = t;
            await whatsappAPI.createTemplate({ ...rest, name: newName, status: 'DRAFT' });
            await fetchTemplates();
            setToast({ message: 'Template duplicated successfully!', type: 'success' });
        } catch (err: any) {
            console.error('Failed to duplicate template', err);
            setToast({ message: err?.response?.data?.message || 'Failed to duplicate template', type: 'error' });
        }
    };
                ...
                <div className="flex items-center justify-between gap-2 sm:gap-4 mb-3 sm:mb-4">
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                        <button
                            onClick={handleSyncStatus}
                            disabled={syncing}
                            className="flex items-center gap-1.5 px-2.5 sm:px-4 py-2 bg-slate-800 text-white text-sm font-semibold rounded-xl hover:bg-slate-900 disabled:opacity-60 transition-colors"
                        >
                            <RefreshCw size={14} className={syncing ? 'animate-spin' : ''} />
                        ...
                        <button
                            onClick={() => { setEditing(null); setShowForm(true); }}
                            className="flex items-center gap-1.5 px-2.5 sm:px-4 py-2 bg-emerald-600 text-white text-sm font-semibold rounded-xl hover:bg-emerald-700 transition-colors"
                        >
                            <Plus size={15} />
    ...
    <button
        ...
        className="flex items-center gap-1.5 px-2.5 sm:px-4 py-2 bg-red-600 text-white text-sm font-semibold rounded-xl hover:bg-red-700 transition-colors"
    >
        <Trash2 size={14} />
        <span className="hidden sm:inline">Delete ({selectedTemplates.size})</span>
        <span className="sm:hidden">Del({selectedTemplates.size})</span>
    </button>
)}
                    </div>
                </div>
                ...
                <div className="flex items-end gap-0 border-b border-gray-100 -mb-px overflow-x-auto scrollbar-none">
                    {VIEW_TABS.map((tab) => {
                        const count = tab.id === 'all' ? counts.all : counts[tab.id as keyof typeof counts];
                        const Icon = tab.icon;
                        const isActive = activeTab === tab.id;
                        return (
                            <button
                                key={tab.id}
                                onClick={() => setActiveTab(tab.id)}
                                className={`flex items-center gap-1.5 px-3 sm:px-4 py-2.5 text-xs sm:text-sm font-medium border-b-2 transition-all whitespace-nowrap ${isActive ? tab.activeClass : 'border-transparent text-gray-500 hover:text-gray-700'}`}
                            >
            ...
            {/* Live status banners */}
            <div className="shrink-0 space-y-2 px-3 sm:px-6 pt-3 sm:pt-1">
                {counts.PENDING > 0 && (
                    <div className="flex items-center gap-3 p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs">
                        <div className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse shrink-0" />
                        <span className="text-amber-800">
...
<div className="flex-1 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 200px)' }}>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
        <div className="bg-white rounded-2xl p-6 w-full max-w-sm mx-4 shadow-2xl">
            <h3 className="font-bold text-gray-900 mb-2">Delete {selectedTemplates.size} Templates?</h3>
            <p className="text-sm text-gray-500 mb-5">This action cannot be undone. All selected templates will be permanently deleted.</p>
            <div className="flex gap-3">
                <button onClick={() => setBulkDeleteConfirm(false)} className="flex-1 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-600 hover:bg-gray-50 font-medium">Cancel</button>
                <button onClick={handleBulkDelete} className="flex-1 py-2.5 bg-red-500 text-white rounded-xl text-sm font-semibold hover:bg-red-600">Delete All</button>
            </div>
        </div>
    </div>
)}
...
function TableView({
    templates, onEdit, onDelete, onPreview, onDuplicate, onSubmitToMeta,
    selectedTemplates, onSelectTemplate, onSelectAll,
}: {
    templates: Template[];
    onEdit: (t: Template) => void;
    onDelete: (id: string) => void;
    ...
    return (
        <div className="p-2">
            ...
            {/* Desktop cards grid - same as mobile view */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {templates.map((t) => {
                    ...
                    return (
                        <div key={t.id} className="bg-white rounded-2xl border border-green-400 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                            {/* Card top: checkbox + name + status */}
                            <div className="px-4 pt-3.5 pb-2.5 flex items-start justify-between gap-3 border-b border-gray-100">
                                <div className="flex items-center gap-3 min-w-0 flex-1">
                                    <input
                                        type="checkbox"
checked={selectedTemplates.has(Number(t.id))}
onChange={() => onSelectTemplate(Number(t.id))}
                                        className="w-4 h-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500 shrink-0 mt-1"
                                    />
                                    <div className="min-w-0 flex-1">
                                        <button
                                            onClick={() => onEdit(t)}
                                            className="text-sm font-semibold text-gray-500 hover:text-blue-800 hover:underline font-mono text-left break-all"
                                        >
                                            {t.name}
                                        </button>
                                        {t.rejection_reason && (
                                            <p className="text-[10px] text-red-500 mt-0.5 line-clamp-2">{t.rejection_reason}</p>
                                        )}
                                    </div>
                                </div>
                                <span className={`inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full border shrink-0 ${sc.badgeClass}`}>
                                    <span className={`w-1.5 h-1.5 rounded-full ${sc.dot} ${t.status === 'PENDING' ? 'animate-pulse' : ''} shrink-0`} />
                                    {sc.label}
                                </span>
                            </div>

                            {/* Meta row: category · type · cost · date */}
                            <div className="px-4 py-2.5 flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-gray-100">
                                <span className={`text-[11px] uppercase font-semibold ${catCfg.class}`}>{catCfg.label}</span>
                                <span className="text-[11px] font-medium text-gray-500 uppercase bg-gray-100 px-2 py-0.5 rounded">
                                    {t.template_type || 'TEXT'}
                                </span>
                            ...
                            <div className="px-3 py-2.5 flex items-center gap-1.5 flex-wrap">
                                ...
                                <div className="ml-auto flex items-center gap-1">
                                    <button onClick={() => onDuplicate(t)} title="Duplicate"
                                        className="p-1.5 text-gray-600 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">
                                        <Copy size={13} />
                                    </button>
                                    <button
                                        onClick={() => onDelete(t.id)}
            ...
            {/* Footer with count */}
            <div className="mt-4 px-2 py-2 flex items-center justify-between border-t border-gray-100 sticky bottom-0 bg-white">
                <span className="text-xs text-gray-400">{templates.length} template{templates.length !== 1 ? 's' : ''}</span>
            </div>
        </div>
    );
...
function TemplatePreviewModal({ template, onClose }: { template: Template; onClose: () => void }) {
    ...
    return (
<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
<div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-0 max-h-[90vh] overflow-y-auto">
