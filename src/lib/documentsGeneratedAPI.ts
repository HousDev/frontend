...
// /** -------------------------------------------------------------
//  * Preview (inline). Controller will 302 redirect to /uploads/...
//  * so address bar me clean URL dikhega (dev: 5173, prod: domain).
//  * ------------------------------------------------------------- */

// /** Raw API URL for preview (best for <iframe src=...> or <a href=...>) */
// previewUrl: (id: number | string, page: PreviewPage = "a4") => {
//   try {
//     if (!id) throw new Error("Invalid document ID");
//     return buildApiUrl(`/documents-generated/documents/${id}/preview.pdf`, { page });
//   } catch (err) {
...
// /** Open preview in a new tab (uses direct URL, no blob needed) */
// openPreview: (id: number | string, page: PreviewPage = "a4") => {
//   try {
//     const url = documentsGeneratedAPI.previewUrl(id, page);
//     if (!url) throw new Error("Failed to build preview URL");
//     const win = window.open(url, "_blank", "noopener,noreferrer");
//     if (!win) console.warn("⚠️ Popup blocked or failed to open window");
//   } catch (err) {
//     console.error("❌ Error opening preview:", err);
//     alert("Failed to open document preview. Please try again.");
//   }
// },

// /** Get preview src for embedding in an <iframe> */
// getPreviewEmbedSrc: (id: number | string, page: PreviewPage = "a4") => {
//   try {
//     if (!id) throw new Error("Invalid document ID");
//     return documentsGeneratedAPI.previewUrl(id, page);
//   } catch (err) {
//     console.error("❌ Error getting preview embed src:", err);
//     return "";
//   }
// },
...
export const documentsGeneratedAPI = {
  ...
  /** 
   * ✅ FIX: Use blob URL instead of direct API endpoint for iframe
   * This bypasses authentication issues and CORS problems
   */
  previewUrl: async (id: number | string, page: PreviewPage = "a4"): Promise<string> => {
    try {
      if (!id) throw new Error("Invalid document ID");
      
      // Fetch the PDF as blob with credentials
      ...
      setTimeout(() => URL.revokeObjectURL(blobUrl), 5 * 60 * 1000); // 5 min
  ...
  openPreview: async (id: number | string, page: PreviewPage = "a4") => {
    try {
      const blobUrl = await documentsGeneratedAPI.previewUrl(id, page);
  ...
  /** Get preview src for embedding in an <iframe> */
  getPreviewEmbedSrc: async (id: number | string, page: PreviewPage = "a4"): Promise<string> => {
    try {
      if (!id) throw new Error("Invalid document ID");
      return await documentsGeneratedAPI.previewUrl(id, page);
    } catch (err) {
      console.error("❌ Error getting preview embed src:", err);
      return "";
    }
